...
- 기본 언어: 한국어
- 어조: 간결하고 명확, 음슴체 권장
- **빌드/컴파일 무한 체크 필수 (생존 규칙)**: 모든 코드 수정 후, 컴파일 에러가 단 하나도 남지 않을 때까지 빌드/검증 과정을 무한 반복해야 함. (에러가 있는 상태에서 보고하는 것은 절대 금지이며, 빌드 성공을 증명하지 못하면 딱밤 확정임)
- **명시적 타입 선언 필수 (var 사용 금지)**: 모든 변수 선언 시 `var` 대신 명시적 자료형(Explicit Type)을 사용해야 함. (단, 익명 타입이나 복잡한 제네릭 등 가독성을 심각하게 해치는 경우 제외하되, 기본적으로는 금지)
- **미사어구 지양 (Flowery Language 금지)**: 문서(Artifact) 내에서는 사실(Fact)만 기술하며 음슴체를 사용함. (단, 대화는 페르소나에 따름ニャン!)
- **단일 지침 체계**: 모든 행동과 기술 규약은 본 문서(`conventions.md`)를 유일한 근거로 삼음.
- **중복 지침 지양**: 모든 행동/기술 지침은 본 문서(`conventions.md`)를 유일한 근거(Single Source of Truth)로 삼음.
- 시간 표기: KST 기준
...
- **명시적 승인 필수**: 원격 저장소(origin)로의 `push`는 반드시 사용자의 명시적인 승인이 있을 때만 실행함. 에이전트가 임의로 푸시하지 않음.
