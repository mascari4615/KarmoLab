...
## 1. 개요
**KarmoToys**는 기존 `KarmoLab`의 모놀리식 구조를 탈피하여, 확장성과 유지보수성을 극대화한 **모듈형 아키텍처** 프로젝트입니다.
`Feature` 단위로 기능을 캡슐화하고, `KarmoToysApp`이 이를 통합 관리합니다.

## 2. 핵심 아키텍처 (Core Architecture)

### 2.1. Feature System
모든 기능(기능 단위)은 독립적인 `Feature` 컴포넌트로 구현됩니다.
...
- **KarmoToysApp.cs**: 애플리케이션의 진입점(Entry Point).
  - **Auto-Feature Discovery**: 실행 시(`Awake`) `EnsureFeatures()`를 통해 필요한 Feature 컴포넌트(`Planner`, `Dashboard` 등)가 없으면 **자동으로 GameObject에 추가**합니다.
  - **Settings Integration**: `KarmoToysSettings` (ScriptableObject)를 통해 초기 설정값을 주입받습니다.
  - **Tab Management**: 등록된 Feature들의 탭 전환을 중앙에서 관리합니다.

### 2.3. Data Management
- **PlannerModel.cs**: 순수 데이터 클래스 (DTO).
- **DataService.cs**: JSON 직렬화/역직렬화 담당.
- **Fail Fast**: UI 바인딩 시 불필요한 Null Check를 제거하여, ID 불일치 등의 문제를 런타임에 즉시 발견하도록 설계되었습니다.
