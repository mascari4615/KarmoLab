...
## 1. 개요
`LifeWeeklyVisualizer` 피처를 `KarmoToys` 아키텍처에 맞춰 구현하기 위한 기술적 설계 문서다냥.
...
### 3.2. 성능 최적화 전략 (성공했다냥!)
- **배율(Scale) 기반 조작**: 5200개의 블록 크기를 일일이 바꾸는 대신, 그리드 부모(`LifeWeeklyGrid`)의 `transform.scale`을 조작하여 GPU 가속을 활용한다냥. 🌬️✨
- **컨테이너 동적 계산**: 스케일링된 그리드가 스크롤 영역을 올바르게 차지하고 중앙에 정렬되도록 컨테이너 크기를 정확히 계산한다냥.
...
### 4.2. UI 구성
- **2단 메뉴바**: 옵션이 많아짐에 따라 '제목/강조 설정'과 '수치/생일 설정'을 분리하여 가독성 확보.
- **중앙 정렬**: `transform-origin: center`와 컨테이너 크기 조절을 조합하여 어떤 배율에서도 그리드가 한가운데에 위치한다냥. 🎯
