...
이 문서는 KarmoToys 유니티 앱에서 **배경이 투명한 새로운 창에 캐릭터를 띄우는 기능**(이하 **Companion Mode**)의 구현 가능성 검토 및 설계 내용을 다룹니다.

## 2. 구현 가능성 분석 (Feasibility Analysis)

### 2.1. 유니티 멀티 윈도우의 한계
Unity의 기본 `Multi-Display` 또는 `WindowManager` API는 물리적인 모니터 확장에 초점이 맞춰져 있어, 단일 모니터 내에서 **독립적인 투명 윈도우**를 여러 개 생성하는 것은 기술적으로 제한적입니다.

### 2.2. 해결 방안: 멀티 프로세스 아키텍처 (Multi-Process Architecture)
**메인 앱(`Planner`)**과 **캐릭터 앱(`Companion`)**을 서로 다른 프로세스(인스턴스)로 분리하여 실행하는 방식을 채택합니다.
같은 실행 파일(`KarmoLab.exe`)을 사용하되, 실행 시 **Command Line Argument**를 통해 모드를 구분합니다.

- **Main Mode**: 기존 플래너/대시보드 앱 (불투명, 일반 윈도우).
- **Companion Mode**: 캐릭터만 표시되는 앱 (투명, Always On Top, 프레임리스).

이 방식은 각 윈도우를 OS 레벨에서 완전히 독립적으로 제어할 수 있어 가장 안정적이고 확실한 해결책입니다.
...
애플리케이션 시작 시(`KarmoToysApp.Awake`), 커맨드 라인 인자를 파싱하여 실행 모드를 결정합니다.
...
Windows OS 환경에서 윈도우 배경을 투명하게 만들기 위해 `User32.dll`과 `Dwmapi.dll`을 사용합니다.

#### 3.2.1. 필수 설정 (Player Settings)
투명화가 정상 작동하기 위해서는 유니티 빌드 설정이 선행되어야 합니다.
...
일반 창(`Windowed`)으로 시작한 뒤, OS 레벨에서 테두리를 제거하고 투명화를 적용하는 방식을 사용합니다.
...
캐릭터는 바탕화면 위에 떠 있어야 하지만, 마우스 입력도 받아야 합니다.

- **Dynamic Click-through (동적 클릭 통과)**:
    - `CompanionFeature`의 `Update` 루프에서 마우스 위치에 캐릭터 UI(VisualElement)가 있는지 `Pick()` 메서드로 확인합니다.
    - **Hit**: `SetClickThrough(false)` -> 입력 차단 (캐릭터와 상호작용).
    - **No Hit**: `SetClickThrough(true)` -> 입력 통과 (바탕화면 클릭 가능).
    - 이 방식은 픽셀 단위 알파 검사보다 성능이 우수하고 구현이 간편합니다.
...
- 서로 다른 Mutex를 사용하므로 두 앱이 동시에 실행될 수 있습니다.
