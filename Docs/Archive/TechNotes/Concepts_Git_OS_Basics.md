...
- **무엇인가요?**: 윈도우의 차세대 터미널인 **PowerShell**에서 실행되는 스크립트 파일. 리눅스의 `.sh` (Shell Script)와 같은 역할을 함.
- **용도**: 복잡하고 반복적인 명령(빌드, 배포, 파일 이동 등)을 파일 하나로 저장해두고, 원클릭으로 실행하기 위해 사용함. (예: `deploy.ps1`)
- **실행 권한 (Execution Policy)**:
  - 윈도우는 보안을 위해 기본적으로 스크립트 실행을 막아둠.
  - 실행이 안 된다면 `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser` 명령어로 권한을 풀어줘야 함.
