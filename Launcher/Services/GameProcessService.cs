...
public sealed class GameProcessService : IDisposable
{
    ...
    public bool StartGame()
    {
        if (IsRunning)
        {
            MessageBox.Show("이미 게임이 실행 중입니다.", "Launcher", MessageBoxButton.OK, MessageBoxImage.Information);
            return true;
        }

        if (!File.Exists(GameExecutablePath))
        {
            MessageBox.Show($"게임 실행 파일을 찾을 수 없습니다.\n경로를 수정하세요:\n{GameExecutablePath}", "Launcher", MessageBoxButton.OK, MessageBoxImage.Warning);
            return false;
        }
        ...
        try
        {
            ...
            MessageBox.Show("게임 프로세스를 시작하지 못했습니다.", "Launcher", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
        catch (Exception ex)
        {
