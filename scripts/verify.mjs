...
/* ★ **어디까지 왔는지 모르면 고칠 값어치를 못 잰다** (2026-08-16). 여기는 첫 빨강에서 멈춘다 . 
   그건 옳다(뒤 단계가 앞 단계 산출물을 쓴다). 그런데 로그에는 멈췄다만 있고 **몇 번째에서**
...
function run(label, cwd, command) {
  ...
  /* ★ **2 = 못 돌렸다(CANNOT-RUN)**. 이 저장소의 규약이다(`run-gates.mjs`, `run-live-checks.mjs`
     ...
     **실패가 아니다.** 여기만 0이 아니면 실패로 두었더니, 검사가 정직하게 못 돌렸다고
...
/* 게이트인 척하는 폴더를 막는다 (2026-08-13 시스템 리뷰).
   ...
   문서도 게이트가 있다고 믿었다. 검사기가 게이트에 안 걸려 있으면 없는 것과 같다
...
//    지운다(윈도, `file:` 의존성). 커밋에는 남아 있으니 아무도 지워졌다고 안 보고,
...
//    karmolab 안에만 세웠다가 재 보니 karmolab-bot 쪽 면이 훨씬 넓었다(292개 중 4개 vs 46개 중 1개).
...
/* ★ **식별자는 영문**. 늘면 여기서 막는다 (2026-08-20).
   룰이 문서에만 있으면 안 지켜진다. 실제로 세션 하나가 주변 코드를 따르라는 일반 지침을
...
//    `test:chat` 이 있다. 그 시험은 karmolab-bot 의 `dist/` 를 읽는데, karmolab-bot build 는 한참
...
// 5. apps/discord-bots/apps/karmolab-bot. build (tsc 타입체크). karmolab-bot 이 main
...
/* ★ **저장소 루트 node_modules 를 보면 안 된다** (2026-08-16 실측). CI 는 루트에 설치하지
   ...
   늘 거짓이었고, karmolab-bot build 가 **한 번도 안 돌았다**. 뒤이어 `test:chat` 도 영원히
   못 돌림이었다(순서를 고쳤는데도 안 고쳐진 이유가 이것이다. 자리가 아니라 조건이었다).
   봐야 할 것은 **karmolab-bot 이 실제로 지어질 수 있나** = 그 워크스페이스의 node_modules 다. */
const botWs = 'apps/discord-bots';
const botReady = existsSync(`${botWs}/node_modules`) && existsSync('apps/discord-bots/apps/karmolab-bot/tsconfig.json');
if (botReady) {
  run('karmolab-bot build (tsc)', 'apps/discord-bots/apps/karmolab-bot', 'npx tsc -p tsconfig.json');
  ...
  run('karmolab-bot 시험 (라우트가 사라져도 잡히게)', 'apps/discord-bots/apps/karmolab-bot', 'npx vitest run');
} else {
  /* 조용한 skip 은 초록으로 읽힌다. 못 돈 것은 못 돌았다고 말한다. 그래야 다음 사람이 본다. */
  console.log(`[verify] ⚠ karmolab-bot 을 못 지었다. ${botWs}/node_modules 가 없다.`);
  console.log('[verify]   그러면 test:chat(창 둘이 실시간 대화) 도 못 돈다. 통과가 아니다.');
  console.log(`[verify]   고치기: cd ${botWs} && npm ci   (CI 는 setup-karmolab 에서 깐다)`);
}
...
//
// 부팅 예산, 성능, 새는 루프는 제품이 고장났다가 아니라 더 나빠졌다를 재는 것들이다.
...
/* ★ **정문을 안 재고 품질을 말할 수 없다** (2026-08-16, 실측). 성능 예산 게이트는 화면 둘을
   본다. 앱 첫 화면과 **도구 한 장**. 그런데 CI 에서는 도구 장이 늘 찍힌 페이지가 없다로
...
/* ★ **첫 화면 미리 그리기도 CI 에는 없다** (2026-08-16, 실측). `apps/blog/` 은
   ...
   이 매 판 찍힌 첫 화면이 없다. 못 돌림으로 빠졌다: 첫 화면이 **미리 그려진 뒤에도 성한지**를
...
const tauriTouched = (() => {
  ...
  /* CI 는 **얕게** 받아 오므로 여기서 `git diff` 를 하면 옛 커밋이 없어 늘 모른다가 된다.
...
// The former KarmoLab companion widget is absent from the shipped registry and bundle.
// The standalone companion remains covered by packages/companion build and tests above.
