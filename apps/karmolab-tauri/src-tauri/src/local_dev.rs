...
/// 이 폴더가 우리 저장소인가 — 서버모니터 설정이 있으면 맞다(그 파일이 없으면 카드가 0개다).
fn looks_like_repo_root(dir: &Path) -> bool {
    dir.join(CONFIG_REL_PATH).is_file()
}

/// 기계가 아는 자리를 짐작한다 (TASK-KL-329). 못 짐작하면 `None`.
///
/// 왜 짐작하나: 개발 판이면 실행 파일이 **저장소 안**에 있고, 깔아 쓰는 판이면 집 폴더
/// 아래 흔한 자리에 있다. 사람에게 경로를 치게 하면 오타가 나고, 오타는 「눌렀는데
/// 안 된다」로만 보인다. 그래서 채워만 준다 — **정하는 것은 사람이 누른다**
/// (짐작이 틀렸는데 조용히 정해 버리면 엉뚱한 폴더에서 굽는다).
#[tauri::command]
pub fn localdev_guess_repo_root(state: State<'_, LocalDevState>) -> Option<String> {
    // 이미 정해져 있으면 그게 답이다 — 짐작보다 사람이 정한 것이 먼저다.
    if let Some(known) = state.repo_root.lock().ok().and_then(|g| g.clone()) {
        return Some(known);
    }

    // ① 실행 파일의 위쪽 — dev 판은 `<저장소>/apps/karmolab-tauri/target/debug/…` 에 있다.
    if let Ok(exe) = std::env::current_exe() {
        let mut cur = exe.parent();
        while let Some(dir) = cur {
            if looks_like_repo_root(dir) {
                return normalize_repo_root(&dir.to_string_lossy()).ok();
            }
            cur = dir.parent();
        }
    }

    // ② 집 폴더 아래 흔한 자리 — 깔아 쓰는 판은 저장소 밖에 있다.
    ...
    for rel in [
        ...
        if looks_like_repo_root(&cand) {
            return normalize_repo_root(&cand.to_string_lossy()).ok();
        }
    }
    None
}
...
#[cfg(test)]
mod tests {
    /// 짐작의 규칙은 한 줄이다: **서버모니터 설정이 있는 폴더**가 저장소다.
    /// 이게 흔들리면 엉뚱한 폴더를 채워 주고, 사람은 그걸 그대로 누른다.
    #[test]
    fn repo_root_is_the_folder_that_holds_the_server_monitor_config() {
        let tmp = std::env::temp_dir().join(format!("kl-guess-{}", std::process::id()));
        let cfg = tmp.join(super::CONFIG_REL_PATH);
        let _ = std::fs::create_dir_all(cfg.parent().expect("parent"));
        assert!(!super::looks_like_repo_root(&tmp), "설정이 없으면 저장소가 아니다");
        std::fs::write(&cfg, "{}").expect("write");
        assert!(super::looks_like_repo_root(&tmp), "설정이 있으면 저장소다");
