...
- KarmoLab 페이지(`apps/karmolab/index.html`)는 **프로덕션 빌드에서** 사이트 루트의 **`/sw.js`(KarmoLab 서비스 워커, 원본 = `apps/karmolab/src/sw.ts`)** 를 등록합니다. 컷오버 전에는 옛 테마의 `/sw.min.js` 였습니다.
...
- **릴리스 체크리스트(버전, 서명, Release, CI)** 는 KarmoLab 앱 **문서 → 로드맵** 탭(`apps/karmolab/js/widgets/docs/roadmap.md` 의 Tauri 데스크톱, 자동 업데이트절)에 모아 두었습니다.
