...
KarmoLab 봇의 AI 비서(DM/전용 채널 자유 대화)와 캐릭터 카드 시스템 사용 설명서.
...
| `ASSISTANT_DEFAULT_CHARACTER` | `byeoru` | `.active.json.default` 가 없을 때 폴백 슬러그 |
...
display_name: 표시 이름
tone: (선택) 말투 요약
speech_style: (선택) 말투 스타일
image_style: (선택) 이미지 생성 스타일
relationship: (선택) 사용자와의 관계
---
```
