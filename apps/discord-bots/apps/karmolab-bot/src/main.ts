...
import { moveCard } from './bot/arcade-invite';
...
import { startFlowReminder } from './services/karmolab-flow-reminder';
...
client.on('messageReactionAdd', (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
  try {
    ...
    getServerStatsRecorder().onReaction({
      ...
      // 봇 발화가 받은 반응은 인기상에서 뺀다. 사람 경쟁이라야 자랑거리가 된다.
...
client.once('clientReady', async () => {
  ...
  /* TASK-KL-210: 배포 파수꾼. 사이트가 며칠째 예 판인데 아무도 모른다를 없앤다.
  ...
  if (characterService) {
    ...
    // 흐름 예약 알림 (TASK-KL-183 B). 서버가 대신 돌지는 않는다. 때가 됐다고 알릴 뿐.
    startFlowReminder(getKarmolabAccountStore(), getKarmolabNotificationStore());
