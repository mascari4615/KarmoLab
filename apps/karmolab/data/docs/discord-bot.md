...
- **슬래시 가드, 로그**: 허용 길드, 채널 ID(`KARMOLAB_BOT_ALLOWED_*`), 선택적 사용 로그(`KARMOLAB_BOT_SLASH_USAGE_LOG`).
...
- **커뮤니티, 게임**. 외부 **캘린더**(iCal URL 등)와 **D-day, 다음 일정** 알림. 버튼 위주 **간단 투표, 참가 신청**(DB 없이 메시지, 반응 집계). **시즌 랭킹** 강화(기존 gamedata, 시즌 이벤트 아이디어와 연계).
...
- **관측**. 대시보드 없이도 관리자 전용 **스냅샷 임베드** 또는 `/debug`: 큐 길이, 음성 세션 수, 마지막 voice close 코드 등. TODO **헬스**와 한 이슈로 묶어도 됨.
- **멀티 인스턴스**. **스테이징 봇 토큰** + 길드 화이트리스트로 프로덕션과 분리(DAVE, 배포 검증, 실험용).

### 문서 보완

- [ ] 아이디어 중 실제 구현에 들어간 것은 **TODO** / **향후 검토**와 역할을 나누고 이 섹션에서 중복 제거
