...
// 이 판(배포)의 표식. 위젯 묶음 주소에 붙여 한 번 받은 것은 그대로로 둘 수 있게 한다.
...
await esbuild.build({
  ...
  ...SAFE_MINIFY,
  bundle: true,
  // ESC 메뉴 모양 (src/lib/esc-menu.css) 은 글자로 묶음. 파일로 내보내면 부르는 곳 없음
  loader: { '.css': 'text' },
  format: 'iife',
  platform: 'browser',
  target: ['es2020'],
...
await esbuild.build({
  ...
  // 커밋도 같이 박는다. 머리띠의 판 표식 배지가 지금 돌고 있는 코드가 어느 판인가를
 ...
 *   따로 낸다. KL-220 이 셸에서 그 script 태그를 빼자 화면이 부르는 곳에서 사라져 컴파일
