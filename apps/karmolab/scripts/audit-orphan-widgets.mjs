...
for (const name of folders) {
  ...
  /* ★ **이름이 어디 나온다고 닿는 게 아니다** (2026-08-16).
     예전 판정은 폴더 밖 어디에서든 이름이 나오면 고아 아님이었다. 그래서
...
/* ★ 톱니. 지금 안 지어지는 것 셋은 지을지 지울지가 사람 결정이라 여기서 못 정한다.
   기준선에 담고 **늘어날 때만** 빨갛다. 갚으면 저절로 줄어든다. 정본 = TASK-KL-319. */
const BASE_FILE = join(app, 'data/unbuilt-widgets.json');
const baseline = new Set(
  existsSync(BASE_FILE) ? /* ⚠ 한때 자료 키가 `목록` 인데 읽는 쪽만 `.list` 로 바뀌어, 이미 적어 둔 셋을
       <b>새로 생긴 고아</b>로 다시 짚었다(2026-08-21). 지금은 자료도 `list` 로 옮겼다 . 
       두 이름이 <b>다시 갈리면</b> 기준선이 통째로 빈 배열이 된다. 자료와 코드는 같이 옮긴다. */
...
if (repaid.length > 0 || process.argv.includes('--write-baseline')) {
  writeFileSync(
    BASE_FILE,
    `${JSON.stringify({ note: '아직 안 지어지는 폴더형 위젯. 늘면 빨강, 지으면(또는 지우면) 저절로 줄어든다', list: namesOnly, updated: new Date().toISOString().slice(0, 10) }, null, 2)}\n`,
  );
