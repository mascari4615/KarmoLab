 ...
 * 하고 WebGPU 도 없다. 그래서 여기서 재는 것은 모델 없이도 참이어야 하는 것이다:
 *
 ...
 * 사각형이라 칠해서 붉다와 원래 빨갛다가 구별이 안 됐다. 검사가 늘 통과하거나
...
/* 조건은 **하나**로. r 이거나 g 이거나 b는 늘 참에 가깝다(느슨한 쪽이 이긴다).
...
if (/못 읽었습니다/.test(vbStatus)) {
  ...
  check(await page.locator('#vbTry').isEnabled(), '첫 장만 해 보기가 눌린다');
