 ...
 *
 * 계기판의 받았는데 안 쓴 코드는 **위젯 단위 근사**다(그려졌나 안 그려졌나). 그보다 정밀한
 * 이 줄이 실행됐나는 브라우저가 페이지에 안 알려 준다. 개발자 도구 전용 통로(CDP)로만 나온다.
 ...
 *
 * 기준선은 `data/coverage-baseline.json`. 없으면 통과가 아니라 **못 돌림**이다.
...
/* 화면 **폭 둘**에서 잰다. 폭이 다르면 쓰이는 규칙이 달라진다. 데스크톱에서만 재면
   `RESPONSIVE` 구역 25KB 가 통째로 낭비로 보이지만, 그건 폰에서 쓰이는 것이다.
   한쪽에서만 재고 안 쓰인다고 적으면 지도가 사람을 엉뚱한 데로 보낸다. */
const VIEWPORTS = [
 ...
 *
 * 파일 단위로 96KB 중 74KB 안 쓰임까지는 알겠는데, 그걸로는 무엇을 뒤로 뺄지 못 고른다.
...
/* 한 파일도 못 받아 온 쪽은 **0 이 아니라 못 잼**이다. 0 으로 적으면 안 쓰는 코드가 없다로
...
if (totals.jsUnused == null) {
  console.log('[coverage]   JS 는 한 파일도 못 받았다. 안 쓰는 JS 가 없다가 아니라 못 잰 것이다.');
}
