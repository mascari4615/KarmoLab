...
//   죽었고, 창이 최소화라 **아무도 죽은 걸 몰랐다**. 화면 보며 고치기가 안 되니 다들 공유
 ...
 * 누르면 없다가 떴다. 화면을 고치는 중에 목록, 상세는 아예 못 열어 봤다는 뜻이다.
...
function resolveFile(rel) {
  ...
  const candidates = [
     ...
     * 화면에 하나도 안 나온다. 고쳤는데 왜 그대로지의 가장 지독한 형태다(실제로 겪었다). */
...
`);
  ...
  // 캐시를 끈다. 개발 중에 옛 파일이 살아 있으면 고쳤는데 왜 그대로지가 된다.
 ...
 * 하나만 고쳐도 200개가 바뀜으로 잡히고, 그러면 매번 셸이 걸려 새로고침으로 떨어진다
 ...
 * 코드라, 고친 사람도 쓰는 사람도 고쳤는데 왜 그대로지를 겪었다(주소 못 찾는 것을 고친 날
...
fs.watchFile(fileURLToPath(import.meta.url), { interval: 500 }, (cur, prev) => {
  ...
  const child = spawn(process.execPath, [fileURLToPath(import.meta.url), String(PORT)], {
    ...
    windowsHide: true,
    // 새 것을 **떼어 놓는다**. 안 그러면 이쪽이 물러날 때 같이 사라져, 다시 뜬다고 찍어 놓고
