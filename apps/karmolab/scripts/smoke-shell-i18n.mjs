 ...
 * 옮겨도 거기엔 닿지 않는다. 실제로 위젯 180개를 다 옮기고 나서도 /en/ 첫 화면의 도구 이름
 ...
 *  , 놀이 이름(스무고개...). 서버(karmolab-bot `/kl/today`)가 보내 준다. 이 저장소 밖이다.
 * 그래서 **지금 보이는 것만 적어 두고, 그보다 늘면 세운다**. 줄면 기준선을 조여 달라고 말한다
 * (`--bless` 로 다시 적는다). 0 이 될 때까지 빨강인 검사는 꺼지기 때문이다.
 *
 ...
 * ⚠ 그런데 이 검사의 통과 조건은 보이는 한글 0개다. 남의 404 화면에도 한글이 0개다.
 *   그래서 <b>남의 서버를 보고도 정상이라 답해 왔다.</b> (오늘 바닥을 넣고서야 드러났다.)
...
/* ★ **그 판을 보는 사람의 브라우저로 연다** (2026-08-19). 여태 시스템 언어(한국어) 그대로
   ...
   안내는 그 언어로 뜬다, 그래야 필요한 사람이 읽는다로 일부러 그렇게 그린다.
...
for (const [code, page] of PAGES) {
  ...
  /* ★ **여기서 0 은 없다가 아니라 안 봤다일 수 있다** (2026-08-21).
  ...
  if (!res || res.status() !== 200 || bodyLen < 200) {
    ...
    console.error('  이건 한글이 안 샌다가 아니라 **아무것도 안 봤다**는 뜻이다. 통과로 안 센다.');
    process.exit(2);
  }
