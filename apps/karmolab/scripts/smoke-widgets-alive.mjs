...
const shard = (() => {
  ...
  return a >= 1 && b >= 1 && a <= b ? { of: a,shardTotal: b } : null;
})();
...
function changedFiles() {
  ...
  try {
    ...
    return combined.split(NL).map((x) => x.trim().replace(/^[A-Z? ]{1,2} /, '')).filter(Boolean);
  } catch {
...
try {
  ...
  for (const id of ids) {
    ...
    if (verdict.dead) dead.push(`${id}: ${verdict.text}`);
  }
} finally {
  await browser.close();
  server.close();
