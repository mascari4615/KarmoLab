/** 흥 의 닫힌 제작 흐름을 실제 브라우저에서 검증한다 (TASK-KL-220). */
...
try { await page.waitForSelector('.hu-root',{timeout:20000}); }
  ...
  const diagnostic = await page.evaluate(() => ({ hash: location.hash, title: document.title, panels: [...document.querySelectorAll('.tab-panel')].map((element) => ({ id: element.id, text: element.textContent?.slice(0, 80), active: element.classList.contains('active') })), scripts: [...document.scripts].map((script) => script.src).filter((src) => src.includes('heung')) }));
...
const clipStartBeforeContext=await page.locator('.hu-clip').first().evaluate((element)=>element.style.left);
await page.locator('.hu-clip').first().click({button:'right'});await page.waitForSelector('.hu-context:not([hidden])');
const contextLabels=await page.locator('.hu-context [role=menuitem]').allTextContents();
const clipStartAfterContext=await page.locator('.hu-clip').first().evaluate((element)=>element.style.left);
...
const multiClipBoxes=await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').evaluateAll((elements)=>elements.map((element)=>{const box=element.getBoundingClientRect();return {left:box.left,top:box.top,right:box.right,bottom:box.bottom};}));
const viewport=page.viewportSize();
const multiLaneBox=await page.locator('.hu-lane[data-kind=midi]').first().boundingBox();
...
const clipCountAfterMultiPasteUndo=await page.locator('.hu-clip').count();
...
const startsBeforeMultiMove=await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.left)));
const dragTarget=await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').first().boundingBox();
/* sticky track head 가 lane 왼쪽을 덮고 오른쪽 8px 은 resize handle 이라, 실제로 clip 본체가
   드러나는 x 를 화면에서 직접 찾는다. */
const grabY=dragTarget.y+22;
const grabX=await page.evaluate(([left,right,y])=>{for(let x=left+6;x<right-14;x+=4){const element=document.elementFromPoint(x,y);if(element&&element.closest('.hu-clip')&&!element.closest('[data-resize]'))return x;}return left+18;},[dragTarget.x,dragTarget.x+dragTarget.width,grabY]);
...
const startsAfterMultiRestore=await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.left)));
const clipCountBeforeMultiDuplicate=await page.locator('.hu-clip').count();
await page.keyboard.press('Control+b');await page.waitForTimeout(120);
const clipCountAfterMultiDuplicate=await page.locator('.hu-clip').count();
await page.keyboard.press('Delete');await page.waitForTimeout(120);
const clipCountAfterMultiDelete=await page.locator('.hu-clip').count();
...
await page.locator('.hu-clip').last().dispatchEvent('dblclick',{button:0});await page.waitForSelector('.hu-editor.is-expanded');await page.click('[data-act=toggle-editor]');
/* 선택하면서 타임라인을 다시 그린 뒤 이미 제거된 요소에 pointer capture를 걸었던 회귀를 직접 밟는다. */
const firstClipBox=await page.locator('.hu-clip.is-selected').boundingBox();
...
await page.locator('.hu-clip.is-selected').dispatchEvent('dblclick',{button:0});await page.waitForSelector('.hu-editor.is-expanded');
const selectedTextAfterDoubleClick=await page.evaluate(()=>window.getSelection()?.toString()||'');
await page.keyboard.press('Escape');
const editorClosedByEscape=await page.locator('.hu-editor.is-expanded').count()===0;
await page.locator('.hu-clip.is-selected').dispatchEvent('dblclick',{button:0});await page.waitForSelector('.hu-editor.is-expanded');
...
await page.locator('.hu-piano').evaluate((element)=>{element.scrollTop=260;});
const existingNoteBox=await page.locator('.hu-note').first().boundingBox();
...
const rollScrollAfter=await page.locator('.hu-piano').evaluate((element)=>element.scrollTop);
...
const audioWave=await page.locator('.hu-lane[data-kind=audio] .hu-wave-svg path').getAttribute('d');
await page.locator('.hu-lane[data-kind=audio] .hu-clip').dispatchEvent('dblclick',{button:0});await page.waitForSelector('.hu-editor.is-expanded .hu-audio-wave');
...
const largeAudioWave=await page.locator('.hu-editor.is-expanded .hu-audio-wave path').getAttribute('d');await page.keyboard.press('Escape');
...
const notesAfterStep=await page.locator('.hu-note').count();
await page.keyboard.press('Backspace');await page.waitForTimeout(120);
const notesAfterStepBack=await page.locator('.hu-note').count();
await page.click('[data-note-act=step]');await page.waitForTimeout(140);
...
const notesBoxSelected=await page.locator('.hu-note.is-selected').count();
const noteTopsBefore=await page.locator('.hu-note').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.top)));
const noteGrabY=(noteBoxes[0].top+noteBoxes[0].bottom)/2;
const noteGrabX=await page.evaluate(([left,right,y])=>{for(let x=left+3;x<right-9;x+=3){const element=document.elementFromPoint(x,y);if(element&&element.closest('.hu-note')&&!element.closest('[data-note-resize]'))return x;}return left+3;},[noteBoxes[0].left,noteBoxes[0].right,noteGrabY]);
...
const noteTopsAfter=await page.locator('.hu-note').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.top)));
const noteDeltas=noteTopsAfter.map((value,index)=>value-(noteTopsBefore[index]??0));
const noteCountBeforeGroupDelete=await page.locator('.hu-note').count();
...
const notePitchesAfterUndoTranspose=await page.locator('.hu-note').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.top)));
const velocityBars=await page.locator('.hu-vel').count();
const velocityHeightsBefore=await page.locator('.hu-vel').evaluateAll((elements)=>elements.map((element)=>parseFloat(element.style.height)));
...
const loopAfterDrag=await page.locator('.hu-loop').evaluate((element)=>({left:parseFloat(element.style.left),width:parseFloat(element.style.width)}));
...
const armedLaneClipsBefore=await page.locator('.hu-lane[data-kind=audio]').last().locator('.hu-clip').count();
...
const guideRemembered=await page.evaluate(()=>localStorage.getItem('karmolab_heung_guide_v1'));
/* 빈 줄 안내는 지금 든 도구를 따라간다. */
await page.keyboard.press('p');await page.waitForTimeout(120);
const hintDraw=await page.locator('.hu-lane-hint').first().textContent().catch(()=>'');
await page.keyboard.press('e');await page.waitForTimeout(120);
...
const helpDialog=await page.locator('.hu-help[role=dialog]').count();
await page.keyboard.press('Escape');await page.waitForTimeout(160);
const helpAfterEscape=await page.locator('.hu-help').count();
...
const loopBeforeSelection=await page.locator('.hu-loop').evaluate((element)=>({left:parseFloat(element.style.left),width:parseFloat(element.style.width)}));
...
const loopDuringSelection=await page.locator('.hu-loop').evaluate((element)=>({left:parseFloat(element.style.left),width:parseFloat(element.style.width)}));
...
const lockedBox=await page.locator('.hu-clip.is-locked').first().boundingBox();
const lockedLeftBefore=await page.locator('.hu-clip.is-locked').first().evaluate((element)=>parseFloat(element.style.left));
const lockedGrabX=await page.evaluate(([left,right,y])=>{for(let x=left+6;x<right-14;x+=4){const el=document.elementFromPoint(x,y);if(el&&el.closest('.hu-clip'))return x;}return left+18;},[lockedBox.x,lockedBox.x+lockedBox.width,lockedBox.y+lockedBox.height/2]);
...
const lockedLeftAfter=await page.locator('.hu-clip.is-locked').first().evaluate((element)=>parseFloat(element.style.left));
const clipsBeforeLockedDelete=await page.locator('.hu-clip').count();
await page.keyboard.press('Delete');await page.waitForTimeout(160);
const clipsAfterLockedDelete=await page.locator('.hu-clip').count();
...
const rowHeightAfter=await page.locator('.hu-track-row').first().evaluate((element)=>element.getBoundingClientRect().height);
await page.waitForTimeout(400);
const rowHeightSaved=await page.evaluate(()=>{const raw=JSON.parse(localStorage.getItem('karmolab_heung_project_v1')||'null');return raw?raw.tracks[0].height:-1;});
...
const lastRowBox=await page.locator('.hu-track-row').last().boundingBox();
...
let presetCount=0, presetApplied='';
{
  await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').first().click();
  await page.waitForTimeout(120);
  presetCount=await page.locator('[data-ins=preset] option').count();
  if(presetCount>1){
    await page.selectOption('[data-ins=preset]','bell');
    /* 자동 저장 250ms 뒤. 그 전에 읽으면 안 붙은 것처럼 보인다 */
    await page.waitForTimeout(600);
    presetApplied=await page.evaluate(()=>{const raw=JSON.parse(localStorage.getItem('karmolab_heung_project_v1')||'null');const track=raw?.tracks?.find((item)=>item.presetId);return track?`${track.presetId}:${track.instrument}:${track.fm?.amount}`:'';});
  }
}
...
if(!mobile){
  ...
  await page.locator('.hu-lane[data-kind=midi]').last().scrollIntoViewIfNeeded();
  await page.waitForTimeout(120);
  laneCountsBeforeCross=await midiLaneCounts();
  const crossSource=await page.locator('.hu-lane[data-kind=midi]').first().locator('.hu-clip').first().boundingBox();
  const crossTargetLane=await page.locator('.hu-lane[data-kind=midi]').last().boundingBox();
  const crossY=crossSource.y+crossSource.height/2;
  const crossX=await page.evaluate(([left,right,y])=>{for(let x=left+6;x<right-14;x+=4){const element=document.elementFromPoint(x,y);if(element&&element.closest('.hu-clip')&&!element.closest('[data-resize]'))return x;}return left+18;},[crossSource.x,crossSource.x+crossSource.width,crossY]);
  await page.mouse.move(crossX,crossY);await page.mouse.down();
  const liveTarget=await page.locator('.hu-lane[data-kind=midi]').last().boundingBox();
