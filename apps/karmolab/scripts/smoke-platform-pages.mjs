 ...
 * , 폰에서 옆으로 넘침
 * 셋 다 HTTP 200으로는 절대 안 잡힌다. 그래서 진짜 브라우저로 열어 본다.
 *
 ...
 * 대신 못 닿아서 못 봤다고 말한다 (통과라고 하지 않는다).
 *
 ...
 * 전부 알맹이가 안 그려졌다로 빨개진다. CI 에서만 나던 그 빨강의 정체가 이것이었다
...
async function check(width, height, label) {
  ...
  for (const screen of SCREENS) {
    errors.length = 0;
    /* ★ **안 그려졌을 때 무엇이 보였는지를 남긴다** (2026-08-13).
     *   CI 에서 다섯 화면이 **전부** 안 그려졌다고 나오는데 내 자리에서는 같은 실주소가 초록이다.
     *   그러면 제품이 고장과 이 판이 막혔다(429, 차단, 로그인 요구)를 가를 수가 없어서
...
/* 봇이 안 답해 못 본 화면이 있으면 **그 사실을 먼저** 적는다. 빨강이 따로 없으면 2(못 잼)로 끝낸다 . 
   통과로 세면 봇이 죽은 날이 초록으로 남는다(안 본 것을 봤다고 적는 자리다). */
