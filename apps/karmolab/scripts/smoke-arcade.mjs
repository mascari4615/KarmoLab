...
async function quitRoom(page) {
  const menu = page.locator('#acMenu');
  if (await menu.isVisible()) await menu.click({ timeout: WAIT });
  await page.locator('#acQuit').click({ timeout: WAIT });
  await page.locator('#acPlay').waitFor({ state: 'hidden', timeout: WAIT });
  await page.locator('#acLobby').waitFor({ state: 'visible', timeout: WAIT });
}
