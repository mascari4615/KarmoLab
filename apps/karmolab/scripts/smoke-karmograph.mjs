 ...
 * 그동안 전부 초록이었다. 없는 것은 검사할 대상이 없기 때문이다. 그래서 띄워서 만져 본다.
 *
 ...
 * 구워 주고 있었는데, 그게 멎은 판에서 나는 **옛 코드를 재고** 초록이라 적을 뻔했다.
 ...
 * 것은 새로 띄운 딴 창과 빌드가 CPU 를 나눠 쓰는 판이라, 바쁜데도 한가함으로 읽혔다
 * (2026-08-13: 그 자로 기계 탓 아님이라 적었다가, 한가한 시간대에 세 판 연속 초록이 나와
 ...
 * 하는 일을 보는 것이지 서랍 몇 번째 줄인가를 보는 게 아니므로, 손잡이를 바로 누른다.
 ...
 * 검사가 보는 건 그 목록이 무엇을 하나이지 탭이 몇 번째 줄에 있나가 아니므로 손잡이를
...
/**
 * **누른다. 안 눌리면 누가 덮었나를 말한다** (2026-08-16, KAR-216).
 *
 * 이 검사가 잡으려는 것이 보이는데 못 누른다인데, 정작 못 눌렀을 때 남는 말은
...
const clickIt = async (p, sel, why2) => {
  try {
    ...
    const inner = await p.evaluate((s2) => {
...
const step = async (name, fn) => {
  try { await withLimit(name, fn); console.log(`  OK   ${name}`); }
    ...
    /* ★ **지나간 바쁨은 사후에 못 잰다. 한 번 더 돌려 본다** (2026-08-16 실측).
       아래 그 자리에서 다시 재 보기는 스파이크가 **아직 남아 있을 때만** 잡는다. 그런데
       빌드 하나가 400ms 튀고 지나가면, 기다림은 이미 끊겼는데 재 볼 때는 한가하다 . 
       그래서 판마다 다른 한 건이 빨강으로 적혔다(실측: 두 판에 칸 후보, 코멘트 개수가
    ...
    /* ★ **못 잰 판을 빨강으로 적지 않는다** (`domain-wm.md` § 관문 ②, ④).
       ...
       있다. 둘을 안 가르면 게이트가 판마다 다른 한 건이 되어 아무도 안 본다(실측 2026-08-13:
    ...
    // 왜 안 눌렸나는 사후에 못 되살린다. 그 자리에서 화면 상태를 한 줄 남긴다.
    ...
    /* ★ 넘어진 자리를 **치우고 다음으로 간다.** 한 항목이 페이지를 딴 데로 보내 놓고 죽으면
       ...
       실측 2026-08-12: 한 번 미끄러지자 그 뒤 열 개가 상태 못 읽음으로 떨어졌다.
...
await step('저장되면 저장됨이 잠깐 뜬다', async () => {
...
await step('얼굴을 안 정한 카드는 **종류 그림**을 단다 (색만으로는 안 갈린다)', async () => {
  ...
  // (2026-08-12 사용자 검토). 그래서 동그라미는 무엇인지를 맡는다.
  await page.waitForFunction(() => {
    ...
    // 이름 첫 글자가 아니라 **그림**이어야 한다(이름이 욘이면 욘이 아니어야 한다).
...
await step('많이 이어진 것을 크게가 실제로 크게 만든다', async () => {
...
await step('링크로 내보내고 그 링크로 다시 받는다', async () => {
  ...
  await page.click('[data-km="more"]');
  // 이 버튼은 눌리면 서랍을 닫는다. 보통 click 은 대상이 사라졌다로 30초를 기다린다.
...
await step('옆 패널. 접힌 다른 목록으로 아홉 패널을 오간다', async () => {
...
await step('점을 빈 곳으로 끌면 새 카드가 생기며 이어진다', async () => {
  // 관계도에서 가장 흔한 동작은 이 사람에게서 뻗어 나가는 또 한 사람인데, 전에는
...
await step('카드를 고르면 판이 안 튄다. 누르려던 손잡이가 도망가지 않는다', async () => {
  ...
  await m.locator('.ck-node').first().click();
  await m.waitForSelector('.ck-link-handle', { timeout: ms(4000) });
  /* ★ 재는 것은 몇 px 움직였나가 아니라 **그 자리를 눌러도 여전히 손잡이인가**다.
     px 문턱은 내가 고른 숫자라 3px 흔들림에 판마다 빨개졌다(실측 1/3). 진짜 지켜야 할 것은
     재고 나서 누르면 빗나가지 않는다이고, 그건 손잡이 제 크기로 재면 된다. */
...
await step('빈 판에서는 툴바가 접힌다 (카드가 생기면 돌아온다)', async () => {
  // 카드가 0장인데 전체 보기, 발표, 찾기가 다 켜져 있었다. 할 수 있는 게 하나뿐인 순간에
  ...
  if (blank > 7) {
    /* 규칙이 안 먹었다와 규칙이 아예 없다는 다른 병이다. 그 자리에서 갈라 적는다.
...
await step('두 장을 고르면 이 둘 사이를 말해 준다', async () => {
  ...
  await m.waitForFunction(
    ...
    throw new Error(`두 장을 골랐는데 이 둘 사이가 없다. 옆 패널: ${t}`);
  });
  await ctx.close();
});
await step('관계망 칸이 숫자 앞에 말 한 줄을 준다', async () => {
  // 연결 3.4, 다리 0.21은 이미 아는 사람에게만 말을 건다. 처음 보는 사람에게는
...
await step('꼬리표를 타이핑하면 제안이 좁아지고 Enter 로 붙는다', async () => {
  ...
  // 중처럼 **글자를 손으로 박으면** 그 글자를 품은 제안이 이 맵에 있어야만 통과한다 . 
...
await step('찾기 → 포커스가 걸린다', async () => {
  // 노드 둘이 서로 이어져 있으므로 1다리로 보면 둘 다 포함된다. 고른 것만으로 본다.
  // 차수 고르개는 툴바에서 **거르기 패널**로 옮겼다 (KL-271 P4). 덜 보기는 한 자리다.
  await openPanel(page, 'filter');
  ...
  /* 찾은 수가 칸 옆에 뜬다. 흐려지는 것만으로는 없다와 아직 안 쳤다가 구별되지 않는다.
     ★ 숫자가 보인다로 재면 안 된다. 이 자리의 판이 무엇이냐에 따라 욘이 0개일 수 있고,
     그때 표시는 없음이라 숫자가 없다. 실제로 그걸로 두 판을 헛되이 빨갛게 만들었다.
  ...
  /* 비우는 순서가 있다. 몇 다리까지가 켜져 있는데 고른 카드가 없으면 그 자리에
     카드를 하나 고르세요가 들어앉는다(그것도 이 자리의 일이다). 둘 다 풀어야 사라진다. */
  await openPanel(page, 'filter');
...
async function grabLink(page, open) {
  /* 클립보드 **읽기**는 권한이 걸려 헛도는 자리가 많다. 그래서 무엇을 복사했는지를
...
/* ★ 발표 줄 단추는 **진짜 클릭**으로 누른다(`dispatchEvent` X).
   `dispatchEvent` 는 그 자리에 뭐가 덮여 있나를 안 본다. 그래서 캔버스 svg 가 발표 줄을
...
await step('발표 모드 진입 / 나가기', async () => {
  // 이 버튼은 눌리는 순간 툴바를 통째로 숨긴다. 보통 click 은 대상이 사라졌다로 보고
...
await step('빈 캔버스에서 예시를 넣으면 그림이 생긴다', async () => {
  await page.click('[data-km="map-new"]');
  await page.waitForFunction(() => document.querySelectorAll('.ck-node').length === 0, null, { timeout: ms(4000) });
  // 견본은 이제 **옆 패널의 갈래 카드**로 깐다. 캔버스 위 큰 버튼은 빈 곳 두 번 클릭을 잡아먹는다.
  await openPanel(page, 'node');
  ...
  // 견본은 **이름이 같으면 한 장**이다(KL-271 P8). 예전엔 소꿉친구, 라이벌이 두 장씩이라 6장이었다.
  ...
  const dupes = await page.evaluate(() => {
    ...
    // 이름을 아직 안 적은 카드(빈 이름)는 같은 이름이 아니다.
...
await step('아직 안 적은 칸을 관계망 칸이 짚어 준다', async () => {
  // 무엇을 더 채워야 하나는 세계관을 짓는 사람이 가장 자주 하는 질문인데, 카드를 하나씩
  // 눌러 보지 않으면 알 수 없었다(KL-271 L6). 관계망의 이어질 법한데 안 이어진 사이의 짝이다.
  await openPanel(page, 'sna');
  await page.waitForFunction(
  ...
  // 관계망 말줄(km-said-line)과 **섞이면 안 된다**. 섞이면 몇 줄인가를 서로 못 센다.
...
await step('공용 글. 승격하면 목록에 뜨고, 둘째 자리에 붙이면 2곳이 된다', async () => {
  ...
  // 그 자리에서 검사를 시작하면 승격 버튼이 없다로 헛 실패한다. 먼저 떼어 내고 시작한다.
  ...
  // 카드만 보고도 나눠 쓰는 글임을 알아야 한다. 모르고 고치면 남의 카드가 바뀐다.
  await page.waitForFunction(
  ...
  // 모아 보기의 증거 = **나머지가 흐려진다**. 노드 수만 세면 아무것도 안 해도 통과한다.
  await page.waitForFunction(
    () => {
...
await step('노드에 칸을 만들면 같은 종류의 다른 노드가 그 칸 이름을 후보로 받는다', async () => {
  ...
  // 이 검사의 주제는 **같은 종류**끼리 칸 이름을 물려받는가다. 그런데 새 노드의 종류는 앞 검사가
...
await step('칸으로 좁히면 그 칸을 적은 것만 남는다', async () => {
  // 앞 검사가 출신: 마계를 적어 뒀다. 좁히면 **적지 않은 노드들이 화면에서 빠져야** 한다.
  await openPanel(page, 'filter');
...
await step('장을 넘기면 화면이 끊기지 않고 미끄러진다', async () => {
  // 목적지에 닿았나만 보면 점프도 통과한다. **중간 프레임이 목적지와 다른지**를 봐야 미끄러짐이다.
  ...
  // 두 장이 **같은 자리**면 화면이 그대로가 당연하다. 사이에 화면을 옮겨 서로 다른 장으로 만든다.
...
await step('둥글게 놓기. 자리가 실제로 바뀌고 아무도 안 사라진다', async () => {
  // 배치는 눌렀더니 아무 일도 안 남이 흔한 자리다. 자리가 **바뀌었는지**와 **개수가 그대로인지** 둘 다 본다.
...
await step('Mermaid 글로 저장하면 문서에 그대로 붙는 코드블록이 나온다', async () => {
  // 값은 파일이 나왔다가 아니라 **붙여 넣으면 그림이 되는가**다. 코드블록, flowchart, 선까지 본다.
  await page.click('[data-km="more"]');
  await page.waitForSelector('[data-km="drawer"]:not(.hidden)', { state: 'visible', timeout: ms(4000) });
  const [dl] = await Promise.all([
    page.waitForEvent('download', { timeout: ms(8000) }),
...
await step('연표로 놓기. 시점 순서대로 왼쪽에서 오른쪽으로 선다', async () => {
  // 자리가 바뀌었다만 보면 아무 배치나 통과한다. **작은 값이 왼쪽인지**까지 본다.
  const box = await page.locator('.km-canvas').boundingBox();
...
await step('공용 글 흩기. 글이 사라지지 않고 자리마다 사본으로 남는다', async () => {
  // 없애기가 빈칸을 남기면 글이 증발한 것처럼 보인다. 그래서 흩은 **뒤에** 글자가 남아 있는지를 센다.
  await openPanel(page, 'notes');
...
await step('첫 화면이 무엇을 만들 건가요 세 갈래를 크게 묻는다', async () => {
  ...
  // 빈 판에서는 **묻는 말이 맨 위**여야 한다. 고르면 여기서 고칩니다가 먼저 오면
  ...
  if (!leadsWithQuestion) throw new Error('빈 판인데 무엇을 만들 건가요가 맨 위가 아니다');
...
/**
 * **덮임 전수 검사** (KL-271, 2026-08-14). 보이는데 못 누른다를 한 항목으로 잡는다.
 *
 ...
 *
 * 화면 읽어 주는 도구에는 글자 칸 단추라고만 들리는 자리가 있으면, 눈으로 안 보는 사람에게
...
await step('다른 탭이 만든 판이 목록에서 사라지지 않는다 (KL-271)', async () => {
  /* 실측 2026-08-14: B 탭에서 새 판을 만들면, A 탭의 다음 저장이 목록을 통째로 덮어 **그 판이
     ...
     목록은 이제 쓰기 직전에 합친다. 내가 지운 것은 지운 표시로 갈라, 도로 살아나지 않는다. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1200, height: 800 } });
  const a = await ctx.newPage();
  const b2 = await ctx.newPage();
  await a.goto(URL, { waitUntil: 'domcontentloaded' });
  ...
  const abox = await untilSettled(a, () => a.locator('.km-canvas').boundingBox());
  ...
  /* 자료가 남아도 **고르개가 낡아 있으면** 분명 만들었는데 없다가 된다 . 
  ...
  /* 지운 판은 **도로 살아나면 안 된다**. 합치기가 내 목록에 없다를 남이 만들었다로 오해하면
...
await step('알림이 소리로도 닿는다 (읽어 주는 도구, KL-271)', async () => {
  /* 실측 2026-08-14: 알림 자리가 **살아 있는 자리(live region)** 가 아니어서, 화면을 못 보는
     사람에게는 저장됨, 만들었습니다, 다른 탭에서 고쳤습니다가 하나도 안 들렸다.
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const cbox0 = await untilSettled(m, () => m.locator('.km-canvas').boundingBox());
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const lab = await untilSettled(m, () => m.locator('.ck-edge-label').first().boundingBox());
...
await step('지운 카드의 번호를 새 카드가 물려받지 않는다 (KL-271)', async () => {
  /* 예전엔 빈 번호를 찾아 줬다. 카드를 지우고 새로 만들면 **지운 카드의 번호가 되살아나서**,
     ...
     **엉뚱한 카드**를 가리켰다. 판마다 여기까지 줬다를 적어 두고 그 뒤 번호만 준다. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1300, height: 850 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const cbox = await untilSettled(m, () => m.locator('.km-canvas').boundingBox());
  ...
  // 다시 열어도 이어서 준다. 여기까지 줬다가 판에 남아 있어야 한다.
  await m.reload({ waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.ck-node', { timeout: ms(8000) });
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const cbox2 = await untilSettled(m, () => m.locator('.km-canvas').boundingBox());
  await m.mouse.dblclick(cbox2.x + cbox2.width * 0.4, cbox2.y + cbox2.height * 0.75);
  await m.waitForSelector('.km-inline', { timeout: ms(5000) });
  await m.keyboard.type('셋째');
  await m.keyboard.press('Enter');
  /* 다시 그리기가 멎을 때까지. 재우고 읽으면 바쁜 기계에서 **옛 목록**을 본다. */
  const ids = await untilSettled(m, () => m.evaluate(() => [...document.querySelectorAll('.ck-node')].map((x) => x.dataset.id)));
...
await step('다른 탭이 고친 공용 글이 이쪽 카드에도 닿는다 (KL-271)', async () => {
  ...
  /* 저장까지 밀어 넣지는 않는다. 그러면 내가 고쳤다로 잡혀 판 따라가기가 멈춘다.
...
await step('안 고친 탭은 다른 탭의 변경을 그대로 따라간다 (KL-271)', async () => {
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const bbox = await untilSettled(b2, () => b2.locator('.km-canvas').boundingBox());
...
await step('폰의 손잡이가 모두 손가락 규격이다 (44px, KL-271)', async () => {
  /* 실측 2026-08-14: 시점 줄 단추 다섯과 다른 목록이 24~38px 이었다. 좁으면 옆 것이 눌려
  ...
  const tight = await m.evaluate(() => {
    ...
    for (let i = 0; i < spots.length; i += 1) {
      for (let j = i + 1; j < spots.length; j += 1) {
        ...
        if (dx <= 3 && dy <= 3) {
          /* ★ **빨갈 때 어디가 얼마나를 같이 낸다** (2026-08-16). 이 줄은 CI 에서만 빨갛고
             내 자리에서는 초록이라(검사 153개) 몇 판째 원인을 못 짚었다. 한글 글꼴을 일부러
             빼고 돌려도 통과했으니 글꼴 탓도 아니다. 이름만 적으면 다음 판도 똑같이 막막하다.
...
await step('폰에서 선 뽑는 점이 손가락 규격이다 (KL-271)', async () => {
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const card = await untilSettled(m, () => m.locator('.ck-node').first().boundingBox());
  await m.touchscreen.tap(card.x + card.width / 2, card.y + card.height / 2);
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const spot = await untilSettled(m, () => m.locator('.ck-link-handle').first().boundingBox());
...
await step('고른 선도 자판으로 지운다 (KL-271)', async () => {
  /* 실측 2026-08-14: 선을 골라 옆 패널까지 떠 있는데 Delete 를 눌러도 아무 일이 없었다
     (카드만 됐다). 자판만 쓰는 사람에게는 고를 수는 있는데 못 지운다가 된다. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
...
await step('모자란 자료로도 판은 열린다 (KL-271)', async () => {
  /* 자료가 조금 모자란 것과 못 읽는 것은 다르다. 네모 없는 묶음 하나에 판 전체가
  ...
  /* 뽑는 문들도 거친 판에서 열어 본다. 읽는 쪽은 늘 있겠지로 자료를 짚는다
...
await step('영어, 일본어 화면에 한국어가 안 남는다 (KL-271)', async () => {
  /* 코드 검사(알맹이)는 **글이 코드에 박혀 있나**를 본다. 그런데 화면에는 다른 길로도 글이 온다 . 
     기본값, 다른 말 묶음, 붙여 만든 문장. 실제로 띄워 보니 이 Person에 대해 적어 두는 것이
...
await step('폰: 보이는데 못 누르는 손잡이가 하나도 없다 (덮임 전수 검사)', async () => {
  ...
  /* ★ **못 눌렀으면 누가 덮었나를 말한다** (2026-08-16). 이 줄이 CI 에서 30초를 기다리다
     죽었는데, 화면에는 시간 초과만 남아 무엇이 위에 있었는지 알 길이 없었다. 내 자리에서는
     늘 눌린다(잰 값: 단추 322,183, 그 자리의 주인 = 단추 자신). CI 는 위 여백이 55px 더 커서
     자리가 달라지는데, 덮은 놈 이름 한 줄이면 그 차이가 곧바로 갈린다.
     이 검사가 잡으려는 것이 바로 보이는데 못 누른다이므로, 못 누른 사실 자체를 증거로 남긴다. */
  try {
    ...
    throw new Error(`더 보기를 못 눌렀다. ${overlay2}`);
  }
  await m.waitForSelector('[data-km="drawer"]:not(.hidden)', { state: 'visible', timeout: ms(4000) });
  await noCovered(m, '서랍');

  await clickIt(m, '[data-km="palette-open"]', '등록부 열기');
  await m.waitForSelector('[data-km="pal-find"]', { timeout: ms(4000) });
  await noCovered(m, '팔레트');
  await m.keyboard.press('Escape');
  await m.waitForTimeout(ms(300));

  await clickIt(m, '.ck-node', '카드');
  await m.waitForSelector('[data-km="node-del"]', { timeout: ms(4000) });
  await clickIt(m, '[data-km="node-del"]', '지우기');
  await m.waitForSelector('[data-km="ask-yes"]', { timeout: ms(4000) });
  await noCovered(m, '되물음');
  await clickIt(m, '[data-km="ask-no"]', '되물음 아니오');
  await ph.close();
});
...
await step('폰에서 서랍이 시트 밑에 깔리지 않는다. 접은 명령에 다 닿는다 (KL-271)', async () => {
  /* 실측 2026-08-14: 서랍은 툴바 안에 매달려 있어(z-index 5 짜리 쌓임 맥락) 아래 시트(960)가
     ...
     툴바가 접은 것들에 닿는 유일한 문인데 그 문의 절반이 죽어 있었다. */
  const ph = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true });
  const m = await ph.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
...
await step('폰에서도 되물음이 손에 닿는다. 아래 시트에 안 깔린다 (KL-271)', async () => {
  /* 실측 2026-08-14: 되물음 상자를 아래 시트와 **같은 층**에 아래쪽으로 두었더니, 폰에서
     지울래요가 화면에 보이는데 손가락은 시트 단추에 닿았다. 보이는데 못 누른다.
     그래서 거기 있나가 아니라 **짚으면 그게 잡히나**를 잰다. */
  const ph = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true });
  const m = await ph.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
  ...
  if (reachable) throw new Error('폰에서 지울래요가 안 잡힌다 . ' + reachable);
  await m.locator('[data-km="ask-yes"]').click();
  await m.waitForFunction(() => document.querySelectorAll('.ck-node').length === 0, null, { timeout: ms(4000) });
  await ph.close();
});
await step('되돌릴 수 없는 일은 얼리지 않고 되묻는다. 기본은 그만 (KL-271)', async () => {
  /* 되묻는 자리는 대개 되돌릴 수 없는 일 앞이다. 브라우저 confirm 은 판을 얼려 **무엇을
     지우려는지 뒤를 못 보게** 만든다. 그게 어느 카드더라를 확인할 길이 없는 채로 고르게
  ...
  // ① 되물음이 뜨고, 브라우저 창은 안 뜬다. 손가락은 그만에 놓여 있다.
  await m.locator('[data-km="node-del"]').click();
  await m.waitForSelector('[data-km="ask-yes"]', { timeout: ms(4000) });
  ...
  if (!onCancel) throw new Error('기본 손가락이 그만에 없다. 되돌릴 수 없는 쪽이 기본이면 사고가 난다');
  ...
  if (await m.locator('.ck-node').count() !== 1) throw new Error('그만을 눌렀는데 지워졌다');
...
await step('안 된 일은 판을 얼리지 않고 말한다. 읽는 동안 화면이 살아 있다 (KL-271)', async () => {
  /* 예전엔 브라우저 alert 이었다: 판이 통째로 얼어 **무엇이 잘못됐는지 뒤를 못 보고** 답해야
     했고, 폰에서는 주소창 이름이 함께 떠 사이트가 하는 말처럼 읽혔다. 여기서는 판 위 말
  ...
  // 백업 파일이 아닌 것을 되살리려 하면 그게 아니다라고 말해야 한다.
...
await step('복사가 막혀도 링크를 직접 가져간다. 판 위 상자에 이미 골라져 뜬다 (KL-271)', async () => {
  /* 예전엔 브라우저 prompt 였다: 판을 통째로 가리고, 긴 주소는 한 줄 창에서 끝이 안 보이고,
     고치라는 건지 복사하라는 건지가 안 읽혔다. 여기서는 **클립보드를 일부러 막고**
...
await step('보기 전용 링크. 손잡이가 사라지고, 내 것으로 복제로 풀린다', async () => {
  // 남에게 보여 줄 때 대부분은 읽히기만 하면 된다. 편집 손잡이가 남아 있으면 받는 쪽이
  // 고쳐도 되나부터 헷갈리고, 고쳐 놓고 원본이 바뀐 줄 안다(사실은 자기 브라우저에만 남는다).
  await page.click('[data-km="more"]');
  await page.waitForSelector('[data-km="drawer"]:not(.hidden)', { state: 'visible', timeout: ms(4000) });
...
await step('보기 전용. 시점은 오갈 수 있고, 시점을 고치는 단추는 없다 (KL-271 X2)', async () => {
  /* 남의 판을 받은 사람에게 시점 오가기는 **읽는 일**이다(1부→2부를 봐야 이야기가 보인다).
     그런데 이름 바꾸기, 지우기는 고치는 손이다. 보이면 고쳐도 되나부터 헷갈리고,
...
await step('꾸미기 규칙. 이 칸이 이 값이면 이 색이 실제로 먹는다', async () => {
  ...
  await page.waitForFunction(
    ...
    null, { timeout: ms(4000) });
  // 카드는 이제 그 종류의 칸을 이미 갖고 태어난다. 첫 칸이 아니라 **이름으로** 찾아야 한다.
...
await step('카드를 다른 카드 위에 떨어뜨리면 이어진다', async () => {
  ...
  //   새 카드가 아니라 남의 카드 이름을 바꾸고, 그 둘이 이미 이어져 있으면 헛 실패한다.
  await page.click('[data-km="map-new"]');
  await page.waitForFunction(() => document.querySelectorAll('.ck-node').length === 0, null, { timeout: ms(4000) });
  ...
  /* ★ **이름칸이 새 카드 것인지 보고 적는다** (2026-08-17, CI 빨강을 로그로 짚었다).
     ...
     **앞 카드 것이 아직 남아 있을 수 있다**. 옆 패널이 새 카드로 다시 그려지는 사이에 적으면
     ...
     이 남아 겹칠 카드를 못 찾았다로 죽었다. 못 만든 게 아니라 **엉뚱한 칸에 적은 것**이다.
  ...
  /* ★ **이름이 카드에 붙기를 기다린다** (2026-08-16, CI 전용 빨강이었다).
     ...
     한 박자 뒤라, 바쁜 기계에서는 그 사이에 찾아 겹칠 카드를 못 찾았다로 죽었다 . 
     못 만든 게 아니라 **아직 안 붙은 것**이다. 시간을 박지 말고 붙을 때까지 기다린다.
     그래도 없으면 **판에 뭐가 있는지 같이 적는다**. 이름만 적으면 다음 판도 막막하다. */
  let from = null;
  let to = null;
  const waitEnded = Date.now() + ms(4000);
  while (Date.now() < waitEnded) {
    from = await boxOf('겹갑');
    to = await boxOf('겹을');
    if (from && to) break;
  ...
  if (!from || !to) {
    const present = await page.evaluate(() => [...document.querySelectorAll('.ck-node')]
      .map((el) => (el.textContent || '').trim().slice(0, 12)).slice(0, 8));
    throw new Error(`겹칠 카드를 못 찾았다. 판에 있는 것: ${present.join(', ') || '없음'}`);
  }
...
await step('본. 한 벌을 떠서 다른 맵에 찍는다', async () => {
  ...
  // 본 이름만 보면 그래서 뭐가 들었더라가 남는다. 든 카드 이름이 함께 보여야 고를 수 있다.
...
await step('이 카드로 오는 링크로 열면 그 카드가 골라져 있다', async () => {
  ...
  // 링크는 **만들자마자 골라져 있는 그 카드**로 뽑는다. 다시 고르러 가면 어느 카드가 골렸나에
...
await step('전체 보기를 누르면 카드가 한 장도 화면 밖에 안 남는다', async () => {
  ...
  await m.reload({ waitUntil: 'domcontentloaded' });
  // ★ **딱 12장이 될 때까지** 기다린다. 하나라도 보이면으로 두면 견본이 함께 깔린 판을
  ...
  /* ★ 여기서는 **다 보이는가**만 본다. 가운데인가는 셈(`cameraForRect`)의 몫이라
  ...
  if (gap.top < -2 || gap.bottom < -2) {
    throw new Error(`전체 보기인데 카드가 화면 밖으로 나갔다. 위 ${gap.top}px, 아래 ${gap.bottom}px `
...
await step('큰 판에서도 끄는 손이 무겁지 않다 (300장)', async () => {
  ...
  // 로컬 실측 0.3~0.9ms. CI 기계는 느리므로 넉넉히. 잡으려는 것은 몇 배로 터졌나다.
...
await step('보이는 단추는 전부 **실제로 눌리는 자리**에 있다', async () => {
  /* 발표 줄 사고(캔버스가 단추를 덮고 있는데 검사는 초록)를 한 번 더 당하지 않으려고,
     ...
     항목마다 눌러 보는 검사와 다르다. 이건 덮였나만 싼값에 전수로 본다. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  ...
  // 첫 그림이 자리를 잡기 전에 재면 아직 안 놓인 것들이 덮였다로 나온다.
  await m.waitForTimeout(ms(900));
  const found = [];
...
await step('카드를 끌면 이웃 카드의 줄에 붙고, 맞춘 줄이 뜬다', async () => {
  // 격자(8px)는 대충 맞음까지다. 폭이 제각각이면 가운데는 격자 위에 없어 영원히 안 맞는다 (TASK-KL-237).
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
  ...
  // 두 번 누르기의 첫 눌림이 고른 것 풀기로 먹히는 판이 있다. 한 번은 다시 눌러 본다.
  ...
  await shutInline();
  await m.waitForTimeout(ms(400));
  // 두 번 누르기의 첫 눌림이 고른 것 풀기로 먹히는 판이 있다. 다시 누르면 **이름칸도 다시 뜬다**.
  ...
  // ★ `waitForSelector` 의 보인다로 세면 안 된다: 세로선은 **폭이 0** 이라 안 보이는 것으로 친다(실측).
...
await step('카드 크기가 자리와 같은 격자에 붙는다 (Alt = 자유)', async () => {
  ...
  // 손잡이를 못 잡았는데 격자에 안 붙었다로 읽으면 엉뚱한 데를 판다. 갈라서 말한다.
...
await step('되돌리기 구멍 감사. 고친 것마다 Ctrl+Z 로 원래대로 (크기, 자리, 이름, 나란히)', async () => {
  // 고칠 수 있다와 되돌릴 수 있다는 따로 논다. 새 편집 기능을 넣을 때마다 되돌리기에
  ...
  await m.waitForFunction(() => document.querySelectorAll('.ck-node').length >= 2, null, { timeout: ms(6000) })
    .catch(async () => {
      /* 판마다 다른 한 건으로 빨개지던 자리. 두 번 눌러 만들기가 가끔 한 장만 만든다.
...
await step('처음 저장될 때 **어디에** 저장됐는지 말해 준다', async () => {
  // 사용자 검토(2026-08-12): 저장됨만 떠서, 방문기록을 지우면 사라진다는 걸 모른 채 쓰게 된다.
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
...
await step('그림을 붙여넣으면 고른 카드의 얼굴이 된다', async () => {
  ...
  // 글을 치는 중에는 가로채지 않는다. 이름 칸에 붙이다 얼굴이 바뀌면 내가 뭘 눌렀지가 된다.
...
await step('고른 카드에 남은 칸 수를 말해 준다 (다 적으면 조용해진다)', async () => {
  // 관계망 칸의 아직 안 적은 칸이 판 전체를 말한다면, 이건 이 카드 한 장이다(KL-271 L5).
  await page.locator('.ck-node').first().click({ force: true });
  await page.waitForSelector('[data-km="edit-label"]', { timeout: ms(4000) });
...
await step('시점 이름은 그 자리에서 고친다 (Esc 는 되돌리고, 딴 데를 눌러도 남는다)', async () => {
  /* 예전엔 브라우저 prompt 였다. 판을 가리고, 고치는 동안 어느 시점을 고치는 중인가가
...
await step('시점을 만들면 판 아래에서 오간다 (안 쓰는 판에는 안 뜬다)', async () => {
  // 1부에서는 소꿉친구, 2부에서는 라이벌을 담으려면 먼저 시점이 있어야 한다(KL-271 X2).
...
await step('선 패널에서 이 시점에서를 고친다 (비우면 원래대로)', async () => {
  ...
  /* ★ **재우지 말고 사라질 때까지 기다린다** (2026-08-16 실측). 여기는 500ms 재운 뒤 읽었는데,
     ...
     (판마다 다른 검사가 터지던 것 중 하나). 바로 위 세 줄은 이미 생길 때까지 기다리기를
  ...
  // 비우면 자리째 지운다. 빈 껍데기가 쌓이면 이 선은 시점 이야기를 한다가 거짓이 된다.
...
await step('내보내기와 저장한 보기도 시점을 따른다', async () => {
  // 글로 옮기는 것도 지금 보고 있는 판이다. 2부를 보며 뽑았는데 1부가 나오면 내가 본 것과
  // 다른 것이 문서에 박힌다. 보기에는 언제를 보고 있었나까지 담는다 (KL-271 X2).
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-root', { timeout: ms(8000) });
  ...
  const back = await untilSettled(m, () => m.evaluate(() => document.querySelector('[data-km="time-go"].is-on').textContent.trim()));
  if (back !== second) throw new Error(`보기를 되살렸는데 시점이 안 따라왔다 (${back} ≠ ${second})`);
  await ctx.close();
});

await step('이 둘 사이와 둘레 보기도 시점을 따른다', async () => {
  /* 2부에는 없는 선을 밟고 한 다리라고 하면, 화면에는 그 길이 안 보인다 . 
  ...
  if (!/이 둘 사이/.test(later)) throw new Error('두 장을 골랐는데 이 둘 사이가 없다');
  if (!/없|안 이어/.test(later)) throw new Error('2부에는 없는 선인데 길이 있다고 한다');
  /* ★ 눌렀으니 됐겠지로 시간만 세면 부하가 걸린 판에서 **아직 2부인 화면**을 읽는다
...
await step('첫 카드, 첫 선까지 드는 손 (처음 연 사람 기준)', async () => {
  /* ★ 이 작업의 진단서에 첫 선 잇기까지의 클릭 수를 재겠다고 적어 두고 못 재고 있었다
     (TASK-KL-271 § 검증). 여기서 **천장을 박는다**. 나중에 한 걸음이 슬쩍 늘면 그 순간 빨강이다.
     세는 것은 사람이 하는 몸짓이다: 두 번 누르기 1, 이름 치고 Enter 1, 점 끌기 1. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-canvas', { timeout: ms(8000) });
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const box = await untilSettled(m, () => m.locator('.km-canvas').boundingBox());
  let acts = 0;
...
await step('마우스 없이도 선을 잇는다 (자판, 화면낭독기 길)', async () => {
  /* ★ 선을 잇는 손은 **점을 끄는 것** 하나로 모았지만(KL-271 R1), 끄는 동작은 마우스에만 있다.
     옆 패널의 연결 시작은 중복이 아니라 **자판, 화면낭독기로 가는 유일한 길**이다 . 
...
await step('같은 자료를 표로도 본다. 줄을 누르면 판에서 골라진다', async () => {
  // 판은 누가 누구와 이어졌나에 강하고 빠짐없이 훑기에 약하다(KL-271 L4, Notion 뷰 계보).
...
await step('보기를 이름 붙여 재우고 한 번에 되살린다 (다시 열어도 남는다)', async () => {
  /* 볼 때마다 거르기를 다시 맞추는 건 매번 같은 일을 손으로 하는 것이고, 그러다 결국 아무도
     안 거른다(KL-271 O2, Kumu 계보). 저장하는 것은 무엇을 보이게 하느냐뿐이다.
     ★ 제 판에서 잰다. 앞선 검사들이 쌓아 둔 저장소(사진, 300장 판, 맵 열아홉)에서 재면
     저장 한계에 걸려 기능이 고장처럼 보인다. 사람이 처음 쓰는 자리를 재는 것이 맞다. */
  const ctx = await browser.newContext({ serviceWorkers: 'block', viewport: { width: 1400, height: 900 } });
  const m = await ctx.newPage();
  await m.goto(URL, { waitUntil: 'domcontentloaded' });
  await m.waitForSelector('.km-root', { timeout: ms(8000) });
...
await step('폰 크기에서 캔버스가 화면 절반 이상이고, 옆 패널은 아래 시트로 뜬다', async () => {
  ...
  // 새 컨텍스트라 판이 비어 있다. **먼저 하나 만든다**(빈 판에서 고르기를 재면 늘 통과한다).
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const up = await untilSettled(m, () => m.locator('.km-side').boundingBox());
  ...
  const cover = await m.evaluate(() => {
    ...
    /* 위젯이 왜 안 밀었는지를 판에 적어 둔다. 그 한 마디가 없으면 CI 에서만 나는
  ...
  /* 재우지 말고 **자리가 멎을 때까지** 기다린다. 바쁜 기계에서는 그 사이 배치가 안 끝나
     옛 자리를 재고, 그 좌표로 누르면 엉뚱한 데를 누른다(2026-08-17). */
  const down3 = await untilSettled(m, () => m.locator('.km-side').boundingBox());
...
await browser.close();
if (frozen) {
  // 그래도 흔들렸다면 **누가 흔들었는지**를 남긴다. 이상하다로 끝나지 않게.
