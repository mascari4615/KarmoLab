...
def main():
    ...
    #
    # 한글을 두 뭉치로 나누는 것은 **두 번 해 보고 두 번 다 접었다.** 다시 하기 전에 읽어라.
    #
    # 1차(먼저 있던 기록): 담은 글자를 정확히 적어 두 뭉치로 나눴는데 브라우저가 뒤 뭉치까지
    #   늘 받았다 — 화면 글에는 뒤 뭉치 글자가 0자였는데도. 257KB → 289KB 로 늘었다.
    #
    # 2차(2026-08-08, 실측으로 다시 봄):
    #   · prod 홈에서 **실제로 그려지는** 한글은 48자다. 화면 밖에 그려지는 글자는 0자,
    #     나머지 260자는 전부 `display:none` 안이라 브라우저가 애초에 안 받는다.
    #     → 즉 나누면 받는 양은 실제로 준다(258KB → 90KB 로 구워 봤다).
    #   · 그런데 **앞 조각이 769자**로 부풀었다. 첫 화면을 그리는 파일들을 통째로 긁으면
    #     한국어 **주석**이 전부 딸려 온다 — 화면에 안 나오는 글자다.
    #   · 그리고 담은 글자를 정확히 적으면 이 목록이 **1.2KB → 17KB** 가 된다. 이 CSS 는
    #     첫 그림을 막는 자리에 있고, 글꼴은 이제 `optional` 이라 안 막는다.
    #     **막는 15.8KB 를 더해서 안 막는 168KB 를 아끼는 거래** — 첫 그림엔 손해다.
    #
    ...
    for key, (_path, css_name, _parts, _ko_policy) in FAMILIES.items():
        for part, pool in pools[key]:
            ...
            # 한글 뭉치도 `swap` (2026-09-21, 전에는 `optional`)
            #
            # `optional`: 첫 그림 뒤 100ms 안에 못 오면 그 방문에는 안 씀. fonts.css 가 첫 그림 뒤에
            # 붙어(CLS 때문에 미룸) 캐시에서 와도 100ms 를 못 맞춤. 결과는 글 본문 한글이 늘 대체 글꼴
            # (Malgun Gothic 92.2%). 사용자 Edge 실측 문단 한글 10자 폭 147.5px (Malgun 160 x 0.922),
            # Pretendard 면 138. 배포 서버(max-age 600)도 같음. 우리 한글 글꼴을 아무도 못 봄
            # `swap`: 도착 즉시 교체. 자리 밀림은 `KarmoSans 자리맞춤` 의 폭과 위아래 지표가 막음 (tools.css)
            display = 'swap'
            css.append(
