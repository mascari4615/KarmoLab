...
const FILE = path.join(root, '../blog/lab/index.html');
 ...
 * 여긴 없으면 건너뜀이었다. 그런데 이 파일을 만드는 곳은 <b>배포 워크플로 한 줄뿐</b>이다
 * (`pages-deploy.yml`. `cp apps/karmolab/index.html apps/blog/lab/index.html`).
 * 그래서 로컬에서도, `verify` 에서도 <b>영영 안 만들어지고</b> 이 단계가 늘 건너뛰었다.
 * 그 여파로 뒤의 `audit:prerender-home` 이 매 판 못 돌림으로 물러났다 . 
...
try {
  ...
  markup = await page.evaluate((live) => {
    ...
    /* 지금 값이 들어가는 칸은 **비운다**. 어제 숫자를 먼저 보여 주는 것보다 빈 자리가 낫다.
       예약 표(`data-reserving`)는 남겨 둔다: 그게 곧 여기 이만큼 온다는 약속이다. */
