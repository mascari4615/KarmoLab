 ...
 * 워크트리, CI)에서 import 만 해도 죽으면 자가 아무 말도 안 하고 죽었다가 된다.
 ...
 *
 * 이 지도의 목적은 내가 쓴 것과 바깥에서 주운 것을 **한 지도에** 놓고
 ...
 * 있다. 쓴 글 ↔ 메모 ↔ 주운 것이 한 판에 놓여야 이건 이미 썼네가 보인다.
 *
...
function collectBlog() {
  ...
  for (const { root, file } of roots.flatMap((r) => walk(r).map((f) => ({ root: r, file: f })))) {
    ...
    /* 달이 00 인 파일이 있다(`2000-00-00-...`). 그건 모른다는 뜻이지 서기 2000년 0월이 아니다.
...
function collect() {
  ...
  for (const src of SOURCES) {
    ...
    /* 소스를 적어 놓고 0편이면 그건 글이 없다가 아니라 **정의가 낡았다**는 신호다. */
...
/* 뜻 재는 모델은 **다국어**여야 한다. 처음에 쓴 영어 전용 모델은 한국어에서 뜻을
   ...
   (그때 고양이↔고양이 0.94로 확인했다고 적었는데, 그건 낱말이 그대로 겹쳐 나온
...
/* **2026-08-21 e5-small 로 갈아타려다 되돌렸다.** 우리 자료로 견줘 보고 정했다
   ...
   **둘 좋아지고 둘 나빠졌다.** 넷 중 셋 이상을 결과 보기 전에 박아 뒀으므로 안 갈아탄다.
   지도의 값어치가 갈래가 만나는 자리와 그림이 거짓말 안 하기에 걸려 있어 더 그렇다.
...
/* 곳간엔 옛 모델 벡터가 같이 산다(열쇠에 모델이 들어 있다). 지도에 **누가 그렸는지**를
   안 적어 두면, 나중에 재는 쪽이 옛 벡터를 집어 들고 안 맞는다고 한다. 실제로 그랬다. */
let usedTier = null;
 ...
 * 그리고 **어떤 값도 맞는 값이 아니다**. 여러 값에서 살아남는 구조가 믿을 만한 것이다.
...
function pacmap2(vectors, opts = {}) {
  ...
  /* 첫 자리 = PCA (정본이 첫 자리가 결과를 크게 좌우한다고 못 박는다). */
...
function linkSuggest(ok, dist, n, edges, okAt, seed = 1234) {
  ...
  /* ★ 사전 문턱 (자와 같은 수 50). 미달이면 재지 않고 자료가 모자란다로 적는다.
  ...
  const candOf = (i, rank) => {
    ...
    for (let j = 0; j < n; j += 1) {
      if (j === i) continue;
      if (known.has(key(i, j))) continue;      // 이미 이은 것은 이어야 할 것이 아니다
      row.push([j, rank(i, j)]);
    }
  ...
  /* 대조군 ① 아무 순서. 몇 등이 뜻을 가지려면 이게 바닥이어야 한다. */
  const rand = score(() => rnd());
  /**
   * 후보가 몇 개나 되나. 찾았다가 아니라 **이만큼 중에 몇 등**이라고 말해야 한다.
   * ★ 처음엔 잘라 낸 뒤 길이를 셌더니 늘 상한(200)이 나왔다. 그건 **내가 자른 수**지
   * 후보 수가 아니다. LBD 의 고질병이 후보 폭증인데 그걸 가리는 셈이었다. 안 자른 수를 센다.
   */
  let poolTotal = 0;
  ...
  /**
   * **보정. 몇 등을 맞을 확률로 바꾸고, 그 확률이 실제와 맞는지 잰다.**
   *
   * 추천 설명의 목표 일곱은 서로 어긋난다(Tintarev & Masthoff). 특히 **설득력을 올리면
   * 효과가 떨어질 수 있다.** 효과는 좋은 결정을 돕는가이지 수락률이 아니다.
   * 사람 없이 그 정신을 지키는 길은 **왜 가까운가를 그럴듯하게 적는 게 아니라
   * 얼마나 맞을 것 같은가를 적고 그 수가 맞는지 재는 것**이다.
   *
  ...
  const scoreProb = (fn) => {
     ...
     * (실제로 우리 것과 늘 같은 확률이 똑같이 0.00007 로 나왔다).
  ...
  return {
    ...
    real, rand,
    /* 판정. 우연을 뚜렷이 넘어야 이어야 할 둘을 화면에 내놓는다. */
 ...
 *
 * ⚠ 우리 자료는 낱말이 아니라 **글**이라 같은 것이 시기별로 어떻게 변했나를 물을 수 없다
 ...
 * 아닌 것으로 두면 최근끼리 뭉친다가 저절로 부풀 수 있다.
 */
const NOVEL_SHARE = 0.2;     // 최근으로 볼 비율 (달을 아는 글 중)
...
function noveltyOf(docs, seed = 606) {
  ...
  /* 최근 = 뒤에서부터 달을 모아 달을 아는 글의 NOVEL_SHARE 를 넘길 때까지. */
  const cnt = new Map();
 ...
 *
 * ⚠ **사생활만 재고 값어치를 안 재면 아무것도 안 내보내면 완벽이 된다.** 그래서 k 를
 * 키우며 **두 곡선을 한 표에** 그린다. 연결 공격 적중률, 사람이 쓴 링크가 같은/이웃 칸에
 * 남는 비율. 둘 다 우연 수준을 나란히 적는다.
 */
/* k 를 **우연 수준에 닿을 때까지** 늘린다. 어느 k 로도 안 된다를 어림이 아니라
...
/**
 * k 마다 **가장 촘촘한 격자**를 고른다. 글의 90% 이상이 k개 이상 든 칸에 남는 것 중에서.
...
function shareGrid(docs, seed = 909) {
  ...
  for (const k of SHARE_KS) {
    ...
    /* k 가 글 수를 넘으면 어떤 격자도 못 선다 (칸 0). 그 행은 잰 것이 아니라 0 을 적은 것이다.
       싣으면 공격 0% < 우연이라는 깨진 수가 표에 남는다. 표본이 1918 → 749 로 줄며 실제로 났다. */
    if (!g.cells) continue;
    const side = g.side;
    const cellOf = (i) => (u[i]
 ...
 * 만으로 가려진 글의 갈래를 맞혀 본다. 잘 맞으면 가려도 소용없다는 뜻이다.
 *
...
function leakOf(docs, seed = 4615) {
   ...
   * 그게 파생 정보도 역변환에 취약하다의 실물이다. 그래서 그 판도 같이 잰다.
   */
  const xyNear = (() => {
 ...
 * 우리 지도의 **x 축 순서**도 후보에 넣는다. 우리 그림이 이미 좋은 정렬인가가 궁금하니까.
 */
function seriationOf(vectors, dist, n, coords, seed = 88) {
   ...
   *
   * ★ 처음엔 블록이 뚜렷한 자료를 눈금으로 썼는데 정렬로 얻는 것이 **−2%** 로 나왔다.
   * 알고리즘 탓이 아니라 **눈금이 틀렸다**: 서로 멀리 떨어진 블록 다섯은 애초에 한 줄로
   * 못 세운다(anti-Robinson 은 줄에서 멀어질수록 닮음이 **단조** 감소를 요구하는데,
  ...
  /* ★ **대조군이 음수로 내려갈 수 있다**. 구조 없는 자료에서는 정렬이 아무 순서보다 **못할** 수도
     있다(2026-08-23 실측: 섞은 자료 −5.3%). 그때 대조군의 두 배는 저절로 참이 되어 문턱이
 ...
 * 그 논문 리뷰에 그대로 적혀 있다. 그래서 **쓰기 전에 우리 자료가 나무 같은가를 잰다.**
...
function deltaOf(vectors, dist, n, seed = 4615) {
   ...
   * 아무 구조도 없을 때 이 자가 내는 값이고, 판정은 그것과 견줘야 뜻이 선다.
   */
  {
 ...
 * 오독한다. 그래서 눈금 표에 같은 표본 수에서 알려진 차원을 얼마로 보나를 같이 싣는다.
 */
 ...
 *
 * UMAP 이 t-SNE 보다 전역 구조를 잘 지킨다는 알고리즘 차이가 아니라 **초기화 차이
 ...
 *
 * ⚠ 논문의 PCA init 을 분산 1e-4 로 축소는 **openTSNE 의 난수 init 규약**에 맞춘 값이다.
...
async function umap2(vectors, opts = {}) {
  ...
  /* 씨앗을 **밖에서 줄 수 있게** 해 둔다. 같은 자료를 여러 판 구워 이 자리가 자료의
     것인지 씨앗의 것인지를 재려면 필요하다(MCE, arXiv 2503.08103). 기본값은 그대로. */
 ...
 * 그런데 안 돌면 그 칸이 **비어 버려서** 자가 안 쟀다로 빨개진다. 그러면 통짜를
 * 돌릴 때마다 빨강이 뜨고, 빨강이 늘 뜨면 아무도 안 본다.
 *
 * 그래서 플래그 없이 구우면 **지난 판의 값을 그대로 물려받는다.** 대신 그 값이 언제
 * 무엇에서 쟀는지(`n`)를 같이 실어서, **글 수가 크게 달라지면 자가 다시 재라로**
 ...
 * 포개기의 목적이 옛 자리에 그대로 얹기인데 마무리가 그걸 도로 흐트러뜨린 것이다.
 *
 ...
 * 즉 지도가 몇 조각으로 끊겼다는 말은 데이터가 아니라 **내가 고른 숫자**가 만든 것이었다.
 *
 ...
 * 우리는 아직 그 추정량을 안 쓴다. 그러니 문헌의 권고대로 한다가 아니라
 * 문헌이 넘어서려 한 방식을 쓰고 있다가 정확한 말이다.
 * 흔드는 법 세 가지: 구간 ±1, 겹침 ±0.05, 글 90%만 다섯 판.
 *
 * 고른 뒤에도 **흔들림 폭을 같이 적는다.** 폭 0 이라야 3조각이다라고 말할 수 있고,
 * 폭이 있으면 화면에도 그렇게 적어야 한다. 안 적으면 손잡이가 만든 모양을 데이터로 읽는다.
 */
/**
 * **렌즈**. mapper 의 나머지 절반 (TASK-KAR-233).
 *
 * mapper 는 어느 방향으로 자료를 훑을까로 시작한다. 문헌의 정본 후보는 밀도 , 
 ...
 * 그 뒤에 어긋남을 잰다. 0 이면 완벽, 1 이면 다 한 점에 몰아둔 것만큼 나쁘다.
 ...
 * 마디마다 그래프 이웃 집합과 그림에서 가장 가까운 같은 수의 마디를 견준다.
 ...
 * 나갔다가 저 갈래로 돌아온다는 순환이 자료에 있다는 뜻이니까. 용수철 배치를 지속
...
function smacofAnchored(P0, D, lambda, iters = ANCHOR_ITERS) {
   ...
   * 깨졌다로 읽으면 멀쩡한 셈을 고장이라 부르게 된다. 실제로 자 하나가 그렇게 빨개졌다.
...
function pickSkeletonParams(pts) {
  ...
  /* **렌즈끼리 견주기.** 덜 흔들리는 쪽 → 덜 달라지는 쪽 → **그림 자 셋의 등수 평균** → 마디 많은 쪽.
     ...
     왜 이 렌즈냐를 사람이 다시 볼 수 있게 한다. 자 하나만 보면 나쁜 그림도 좋다고 나온다. */
 ...
 *
 * 같은 마디의 잣대(자카드 문턱)도 **박지 않고 쓸어서 고른다.** 0.5 로 박아 뒀더니
 ...
 *
 * 그리고 **대조군 값을 같이 싣는다.** 이게 핵심이다. 85% 살아남았다는 말은
 * 마구 섞어도 64% 는 살아남는다와 나란히 놓지 않으면 아무 뜻이 없다.
...
function nodeConfidence(pts, nodes, lens, bins, overlap) {
  ...
  /* **문턱을 박지 않는다.** 반도 못 버틴 마디로 세니 0개였는데, 스무 판을 다 버티지
...
function binGroups(pts, bins, overlap, min, shift = 0) {
  ...
  /* `shift` = **눈금판 자체를 옆으로 민다**(구간 너비의 몇 분의 몇). 0 이면 옛 그대로.
     이걸 안 흔들면 글을 좀 빼도 마디가 남나는 늘 남는다다. 마디를 정하는 것이
 ...
 *
 * ★ 처음엔 절반 넘게 겹치면 같은 것으로 **다 이어 붙였다**. 그러면 갈라짐이 통째로
  ...
  for (let k = levels.length - 1; k >= 0; k -= 1) {
    ...
    for (const [p, list] of kids) {
      ...
      for (const y of list.slice(1)) {
        /* 작은 자식은 여기서 죽는다. 이 눈금부터 갈라져 나온다는 뜻이다. */
 ...
 * **붙들고 볼 그 하나의 답이 없어** 불확실성을 안 무시하게 되기 때문이다.
 *
...
function buildSkeleton(docs, coords) {
  ...
  /* 손잡이는 **재서 고른다**. 손으로 박아 둔 12, 0.3 은 40자리 중 29위였고, 흔들면
     조각 수가 3까지 벌어졌다. 그 그림의 몇 조각은 데이터가 아니라 그 숫자였다. */
  ...
  const before = {
    ...
    /* **원래 자리를 같이 싣는다.** 옮겼다는 표시만 실으면 그 표시를 지우는 것으로
       숨길 수 있다(자가 그걸 못 잡는 걸 망가뜨림 판이 보여 줬다). 자리를 실어 두면
       자가 **스스로 재서** 옮겨 놓고 입 다물었다를 잡는다. */
  ...
  /* ★ **대조군 없이는 고리가 있다가 아무 뜻이 없다.** 겹치는 구간으로 잇는 셈이라
  ...
  return {
    ...
    h1: {
      ...
      /* 자리를 마구 섞은 지도의 고리 수. 이것 없이 고리가 있다는 아무 뜻이 없다. */
      rand: h1Rand,
    },
    /* 어떤 손잡이로 그렸고 흔들면 얼마나 달라지나. **화면에 같이 적으라고** 싣는다. */
    params: { bins, overlap, min: MAPPER_MIN, lens },
    /* 렌즈 넷을 같은 잣대로 견준 표. 왜 이 렌즈냐를 자가 다시 볼 수 있어야 한다. */
 ...
 *
 * 원래 목적이 쌓아만 두고 다시 안 봄이다. 다시 꺼내기의 정석(간격 반복)은
...
/**
 * 글의 **생일**. 처음 담긴 날. 관심이 어디로 옮겨갔나를 보려면 마지막으로
 ...
 *
 * ⚠ 그리고 **앞 시기 정보만** 쓴다. 지난 바퀴에 이웃이 같은 시기에 움직였나로 80% 를
...
function doiRevisit(docs, events, novelty, seed = 4242) {
  ...
  return {
    ...
    /* 판정. 우연, 바탕을 뚜렷이 넘어야 지금 손대는 것 주변을 내놓는다. */
...
/**
 * **쓰이는가. 지도가 다시 손댈 글을 미리 짚나** (Barreau & Nardi, SIGCHI Bulletin 1995).
 *
 ...
 *
 * 우리 지도는 정확히 묵힌 것을 위한 정교한 분류 체계다. 문헌대로면 **안 쓰인다.**
 * 그래서 지도를 **일깨움 도구**로 볼 수 있는지 잰다. 사람 없이, git 이 정답을 준다:
 * **다시 손댄 글**이 곧 사용자가 실제로 돌아온 글이다.
 *
...
function revisitCheck(docs, touched, novelty, seed = 77) {
  ...
  const older = [];
  for (let i = 0; i < n; i += 1) {
    if (!born[i] || recent.has(born[i])) continue;    // 최근에 태어난 것은 다시가 아니다
    older.push(i);
  }
  ...
  /* 나이별 재방문율. 묵힌 것은 거의 안 본다가 우리에게도 맞나. */
  const byAge = new Map();
...
function markBuried(docs, edges) {
  ...
  /* 위 25% 만 오래된 것으로 본다. 다만 **바닥을 깐다**. 대부분이 최근이면
     넉넉잡아 여드레짜리도 오래됐다가 되어 버린다(실제로 8일이 나왔다).
...
function findEdges(docs) {
  ...
  const seen = new Set();
  const edges = [];
  /* ★ **누가 쓴 링크인지 남긴다.** 링크의 때는 두 끝의 생일이 아니라 **그 링크를 적은
 ...
 * 딴 데로 튀어 방금 저기 있던 게 어디 갔지가 된다. 나무여야 지도다.
 *
 ...
 * ★ **뽑은 자리를 밖에 알린다.**
 *
 * 굽는 쪽과 자가 같은 1200개를 쓴다고 믿었는데 실은 **서로 다른 1200개**였다 . 
 ...
 * 관문 세계가와 청크 BT가 어떻게 다르냐고 물으면 지도가 답을 못 했다.
 *
 ...
 *
 * 두 가지를 못 박는다:
 * , **침입자는 전체 빈도를 맞춰 뽑는다.** 안 그러면 드문 것 고르기로 풀려 시험이 순환한다
 *    (침입자를 여기선 드문 말로 뽑아 놓고 드문 것을 답으로 받는 셈).
...
async function wordIntrusion(level, ok, vecs, embed) {
  ...
  for (const t of trials) {
    ...
    /* **순환 검사**. 빈도가 가장 낮은 것으로 풀면 몇 판이나 맞나. 여기가 높으면
       이 시험은 뜻을 재는 게 아니라 드문 것 고르기다. */
    ...
    /* 반대 방향도 본다. 빈도가 **0** 이면 드문 것 고르기로는 안 풀린다는 뜻이지만,
       그 자리에 흔한 것 고르기가 들어앉았을 수 있다. 둘 다 찍기 근처여야 한다. */
 ...
 *  ② **맞음**. **남겨 둔 글**(대표에 안 쓴 글)에 그 갈래를 적용해 여기 드나를 가른다.
...
function topWordsByGroup(groups, k = 10) {
  ...
  /**
   * ★ **이쪽만 쓰는 말은 재서 거른다** (2026-08-23).
   *
   * 전에는 c-TF-IDF 점수와 절반 넘는 덩어리에 나오면 버린다만으로 골랐다. 그건 **덩어리 수**를
   * 셀 뿐 **얼마나 자주 나오는지**를 안 본다. 그래서 화면의 견주기 칸에 이쪽만 쓴다고 올려
  ...
  return tf.map((c, gi) => {
    ...
    for (const [w] of ranked) {
      ...
      /* 겹치는 말은 하나만. 근본 코어, 다음 근본, 다음 근본 코어가 다 뜨면
...
function nameFit(groups, names, words) {
  ...
  for (let i = 0; i < groups.length; i += 1) {
    ...
    /* **둘 다 -1 이면 못 잰 것이다**. 이름 말과 무리 말이 어디서도 같은 창에 안 든다는
       뜻이라, 제 무리가 더 낫다/아니다를 말할 근거가 없다. 통과로도 실패로도 세지 않는다.
 ...
 *
 * c-TF-IDF 는 이 무리에서만 자주 나오는 말을 고른다. 그것만으로는 그 무리를 대표하지
...
function refineNames(groups, names, words) {
  ...
  for (const i of order) {
    ...
    /* **찜한 이름은 후보에서 뺀다. 제 원래 이름도 예외가 아니다.** 처음엔 원래 이름만
       봐주게 뒀더니 층 하나에서 관문이 두 무리에 붙었다(이름이 겹치면 지도를 봐도 소용없다).
 ...
 *
 * 이 덩어리에서만 자주 나오는 말을 고른다. 두 낱말짜리가 한 낱말짜리와 점수가
 * 비슷하면 **두 낱말짜리를 고른다**. 관문보다 국경 관문이 어디인지 말해 준다.
...
function nameAllByWords(groups) {
  ...
  for (const [gi] of order) {
    ...
    for (const [w] of ranked[gi]) {
      ...
      /* 앞선 덩어리가 쓴 낱말이 끼면 넘어간다. 국경 관문과 관문 세계가 나란히
         있으면 어느 쪽이 어딘지 모른다. */
 ...
 * 그냥 자른 것이다. 그런데도 화면은 그걸 덩어리라 부르고 견주기까지 붙여 놨다.
 *
 * 그래서 두 가지를 한다: ① 성긴 층을 **봉우리에서 고른다**(박아 두지 않는다)
 * ② 층마다 실루엣을 **실어 보낸다**. 낮은 층에서는 화면이 덩어리 대신 구획이라 말한다.
 * 기능은 그대로 두고 **말만 정직하게** 한다.
 *
 * ⚠ 실루엣은 볼록한 무리를 전제하고 고차원에서 약해진다(문헌 명시). **절대 판정이 아니라
 * 상대 신호**로 쓴다. 봉우리를 고르는 데 쓰고, 0.03 이니 나쁘다로 단정하지 않는다.
 */
/** 이보다 낮으면 덩어리가 아니라 **구획**이라 부른다. 화면도 그 말을 쓴다. */
const SIL_REAL = 0.15;
 ...
 * 연속된 구름에 빽빽한 자리. 실루엣 0.03~0.06 이 무리가 없다가 아니라
 ...
 *
 * DBCV 논문이 손잡이 고르기에 이 지표를 쓸 수 있다고 못 박아 뒀다. 그래서 손으로
 * 고르지 않고 **DBCV 가 가장 높은 자리**를 쓴다(뼈대 손잡이 때와 같은 규율).
 * 곡선을 통째로 실어 보낸다. 자가 다시 볼 수 있어야 골랐다가 된다.
 */
...
/**
 * **H0 지속 막대**. 진짜 덩어리가 몇 개냐를 **나누지 않고** 답한다 (지속 호몰로지).
 *
 * 실루엣(거리), DBCV(밀도), HDBSCAN(안정성)은 다 이 **나눔**이 좋은가를 잰다. 지속
 ...
 * 그 위가 오래 버틴 조각이다. 곡선을 통째로 실어 자가 다시 볼 수 있게 한다.
 */
/**
 ...
 *
 * 우리 판정은 여태 **낙차 1.5배 이상** 이었다. 이 세션 내내 문턱은 재서 고르라고
 ...
 *
 * ★ 우리 H0 는 여태 **순수 거리**였다. 주석에 상호도달거리가 아니다라고 일부러 적어
 * 두기까지 했다. 그런데 우리 자료는 HDBSCAN 이 **75%를 어디에도 안 붙는다**고 하는
 * 이상치 투성이다. 정본이 이럴 땐 쓰지 마라는 바로 그 상황이었다.
 *
...
function bootBand(bars, dist, n, seed0 = 8123) {
  ...
  const band = 2 * c;
  /* ★ **띠를 그대로 대각선에서 얼마나 먼가로 쓰면 우리 자료에선 안 된다.**
     재 봤다: 지어낸 세 덩어리에서도, 아무 구름에서도 **똑같이 185개**가 띠 밖으로 나온다.
     까닭 = 우리 H0 는 **최소신장나무의 가지**라, 점마다 가장 가까운 이웃까지의 거리만큼
...
function h0Bars(vecs, D = null) {
  ...
  const stat = {
    ...
    boot,
    /* 이상치에 덜 흔들리는 답. 손잡이 m 사다리. 거리로 본 답과 **둘 다** 싣는다. */
...
/** 이보다 낮으면 밀도로 봐도 덩어리가 아니다. 지어낸 눈금에서 갈린 셋 0.91, 자른 구름 -0.04. */
const DBCV_REAL = 0.3;
...
function pickCoarse(vecs, fine, sizes, fallback = 6, candidates = [4, 5, 6, 7, 8]) {
  ...
  /* **두 표본이 같은 자리를 가리키고, 둘 다 뚜렷할 때만** 층을 옮긴다. 아니면 손대지 않는다.
     평평한 곡선에서 봉우리를 고르는 건 잡음을 좇는 것이고, 숫자를 바꿔 놓고 재서 골랐다고
     말하는 게 더 나쁘다. 실측: 쏠림을 뺀 벡터에서 후보들이 0.03~0.06 로 다 붙어 있다
     (빼기 전 원 벡터로는 0.24 로 보이지만 그건 모두가 공유하던 방향이 만든 착시다). */
 ...
 *
 * 이미 있는 묻힌 글은 *시간* 기준이다. 오래 안 건드렸고 아무도 안 부른다.
 ...
 * 외장뇌 글이었다. 재료가 없으니 당연히 혼자 떨어진다. 그건 본문이 없다를 다시
 * 찾아낸 것이지 새 렌즈가 아니다. 400자로 자르니 묻힌 글과 겹침이 6/12 → 1/12 로
 * 떨어졌다(2026-08-21 실측).
 *
...
function lonelyPerDoc(ok, docs) {
  ...
  /* ★ 겹침이 1/3 을 넘으면 **렌즈를 접는다** (자와 같은 문턱). 묻힌 글을 다시 비추는
     ...
     코퍼스 개편(1918→749, 링크 877→52)으로 부르는 글 없음이 흔해지자 실제로 넘었다. */
 ...
 *
 * 이 지도의 목적은 주운 것과 쓴 것이 겹치는 자리다. 그런데 겹침을 재는 자가
 * 덩어리 순도밖에 없었다. 덩어리를 어떻게 나누느냐에 딸려 있고, 경계에서 튄다
 * (실제로 한 번 뒤집혔다: 89, 86% → 54, 53% 인데 나빠졌다로 읽혔다).
 *
...
/** 이웃 갈래가 이만큼 되면 만나는 자리로 친다. 1.5 = 8개 중 둘째 갈래가 두엇 이상. */
const MEET_AT = 1.5;

/**
 * 자리마다 **이 지도가 여기서 얼마나 정직한가**를 매긴다 (TASK-KAR-233).
 *
 * 384차원을 2차원으로 줄여 그리면 **반드시** 거짓말이 생긴다. 문제는 거짓말을 하나가
 ...
 *
 * ★ 우리는 **찢김만 재고 있었고, 그걸 옆에 있어도 남남이라 불렀다.** `honest` =
 ...
 *
 * 우리는 안 갈린다를 **한 수**로만 말해 왔다(실루엣 0.042, DBCV −0.235, 꿋꿋함 0.121).
 * 정본은 갈려 보이느냐가 **여러 요인의 결과**라고 한다. **무리 안**(크기, 퍼짐, 밀도, 
 * **늘어짐**, 이상치)과 **무리 사이**(**겹침**, 굽음). 고치려면 겹쳐서인지 퍼져서인지를
 * 알아야 한다.
 *
 ...
 * **초기화 사다리를 굽고 재서 고른다.**
 *
 * 주 판정은 **전역 거리 상관 r_global** 이지 떨림이 줄었다가 아니다. init 을 못 박으면
...
async function initLadder(vectors, params, opts = {}) {
   ...
   * init 을 한 귀퉁이에 몰아넣은 극단 배치로 주고 구운 판이 난수 init 판과 **달라야** 한다.
 ...
 * 초기 난수 때문에 같은 설정에서도 매판 다른 국소최적에 빠지므로, 판 하나를 지도라
...
function fitTo(A, B) {
  ...
  /* 크기도 맞춘다. 안 맞추면 더 크게 그린 판이 떨림으로 잡힌다. */
  let num = 0;
  for (let i = 0; i < n; i += 1) {
    const bx = B[i][0] - mB[0]; const by = B[i][1] - mB[1];
 ...
 * 대신 **언제 쟀는지**를 같이 실어서, 자가 옛날 값으로 말하고 있나를 볼 수 있게 한다.
 */
async function seedWobble(vectors, params, opts = {}) {
   ...
   * 판을 늘리면 **가운데 자리가 모이나**.
   *
   * ★ 처음엔 앞 k판끼리의 평균 거리를 쟀는데 그건 **판 수와 무관한 모집단 값**이라
...
async function metricZoo(vectors, docs, params, opts = {}) {
  ...
  const measure = (pts, idxSeed) => {
    ...
    /* stress-1. 배율을 맞춘 뒤 재야 더 크게 그린 판이 늘어남으로 안 잡힌다. */
    let num = 0; let den = 0;
 ...
 * 그 알파도 전부 그리기도 한 번도 안 쟀다. 손으로 고른 상수다.
 *
      ...
      for (let e = 0; e < adj[i].length; e += 1) {
        /* EI = 이 이음을 따라가기 싫은 정도.
           ...
           **감쇠가 사실상 없었다.** 그러면 확산은 그냥 α^홉 이 되고, α<0.5 면 아무것도 안
           ...
           1/EI 가 [0.5, 1] 쯤에 놓이게 한다. 그래야 따라가기 싫은 이음이 뜻을 갖는다. */
 ...
 * 차수를 안 지키면 많이 링크된 글이 잘 잡힌다와 구별이 안 된다.
 */
...
function doiEval(docs, ok, dist, n, edges, okAt, seed = 233) {
  ...
  /* ③ 화면 밖 신호. 표시한 마디마다 숨은 이웃 수가 맞나, 확장 방향 top-3 이 맞나. */
...
/**
 * **dip 검정. 이 둘은 정말 갈리나에 p 값을 붙인다** (Hartigan & Hartigan 1985).
 *
 * ★ 우리 주장 구획이지 무리가 아니다의 근거는 지금까지 전부 **문턱을 손으로 고른 자**
 ...
 * 최대 어긋남을 잰다. 그 중 **가장 작은 것**이 dip 이다. 가장 잘 맞는 단봉 함수와의 거리.
 */
function dipStat(xs) {
  const n = xs.length;
  if (n < 8) return 0;
 ...
 * 아무 점 무더기라도 가장 갈라 보이는 방향으로 투영하면 두 봉우리처럼 보인다. 그래서
 * 대조군 둘을 같이 낸다:
 ...
 * 거리를 **같은 크기 아무 무리**의 평균 거리로 나눠 본다(1 이면 아무 무리와 다를 바 없다).
 ...
 *
 * , **상호 근접도(정규분포판)**. x, y 둘 **다에게서** 이 거리보다 먼 점이 얼마나 되나를
...
function honestyPerDoc(docs, coords, screenK = 24, vecOf = null) {
  ...
  for (const [i, p] of pts) {
    ...
    /* **나머지 반쪽은 여기서 못 잰다.** 화면 이웃이 스물넷인데 여덟째 닮은 글을 잣대로
 ...
 * (여덟째 닮은 글보다 덜 닮았나로 재면 완벽한 지도도 84% 가 걸린다. 스물넷을 여덟으로
...
let serOut = null;     // 어긋남 요약. 찢김, 거짓 이웃 (CheckViz)
   ...
   * 자들에겐 **블로그 글만 생일이 있었고**, 이어야 할 둘을 시간으로 자르려 했을 때
  ...
  if (!flag('--no-embed')) {
    ...
    if (ok.length >= 3) {
      ...
      /* **씨앗 밴드**. 같은 손잡이로 씨앗만 바꿔 여러 판 굽고 이 자리가 자료의 것인지
         난수의 것인지를 잰다. 판 하나가 20여 초라 `--씨앗` 을 줄 때만 돈다. */
      ...
      if (flag('--씨앗') || flag('--seeds')) {
        try {
          ...
          console.log(`[atlas]   ★ **화면 이웃이 판마다 그대로인 비율 ${wobbleOut.keep}**`
            ...
            + `. 이게 높은데 떨림이 크면 이웃은 자료의 것, 큰 배치는 씨앗의 것이다`);
      ...
      /* 자리가 정해졌으니 이제 여기서 얼마나 정직한가를 매길 수 있다. */
      ...
      /* **어긋남의 나머지 반쪽**. 화면 이웃 중 진짜 순위가 밖인 것(거짓 이웃). 거리행렬이 필요하다. */
      {
        ...
        /* **관심도(DOI)**. 전부 그리고 흐리게 대신, 예산 안에서 연결된 것만 남기면
           초점에서 **2홉 이상** 떨어진 사람이 손으로 쓴 링크를 얼마나 건지나. */
        try {
      ...
      /* **성긴 층은 재서 고르되, 곡선이 평평하면 손대지 않는다.** 곡선을 통째로 실어
         보낸다. 어떻게 골랐는지를 자가 다시 볼 수 있어야 고른 게 된다. */
      ...
      for (const target of LEVELS) {
        ...
        /* 이 층이 **정말 무리인가**를 같이 싣는다. 낮으면 화면이 덩어리라 안 부른다.
           **성질이 다른 자 둘**로 잰다: 실루엣(거리), DBCV(밀도). 하나로만 재면
           이 자로는 못 잰다와 무리가 없다를 못 가른다. */
        ...
        /* **왜 안 갈리는지**. 한 수 말고 요인 이름으로. 화면 좌표에서 잰다(보이느냐가 물음). */
        ...
        /* **p 값**. 지금까지 구획이지 무리가 아니다의 근거는 전부 문턱을 손으로 고른
      ...
      /* **진짜로 뭉친 자리**. 어디에도 안 붙는다는 답이 있는 나눔(HDBSCAN).
         층은 그대로 둔다. 자 둘이 구획이지 무리가 아니다라 했으니, 그 위에
      ...
      /* **낱말 침입자**. 나눔이 좋은가가 아니라 이 말들이 읽히나를 묻는다.
  ...
  /* **묻힌 글 표시가 끝난 뒤에** 잰다. 그래야 시간 기준과 얼마나 겹치나를 셀 수 있다. */
   ...
   * 함께 나간다. 제목을 가려도 **이웃이 갈래를 말해 주는지**를 재서, 가리면 안전하다는
  ...
  /* **쓰이는가**. 지도가 다시 손댈 글을 미리 짚나. git 이 정답을 준다. */
  try {
