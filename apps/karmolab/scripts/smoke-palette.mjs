 ...
 *
 * 왜 있나: 이 화면은 첫 화면의 본체다. 여기가 조용히 죽으면 앱에 도구를 찾을 길이
 * 하나도 안 남는다. 고치기 전 상태로 되돌아가는데, 화면은 멀쩡해 보인다(입력칸은 그려지고
 * 글자도 쳐진다, 목록만 안 뜬다). 그래서 그려졌나가 아니라 **쳐서 열리나** 를 본다.
 *
 * 보는 것:
...
/* ── ① 첫 화면에 찾는 입력이 있다 ───────────────────────────
 * 예전에는 포커스까지 쥐어야 한다로 봤다. 그런데 첫 화면이 바뀌어 입력이 카드, 통계
 ...
 * 박아 두면 남의 정상적인 추가가 남의 잘못으로 잡힌다. 사라지지 않았나만 본다. */
...
if (!titles.some((t) => t.includes('이미지'))) {
  problems.push(`이미지 압축으로 이미지 도구가 안 나온다. 나온 것: ${titles.slice(0, 3).join(', ') || '없음'}`);
}
...
if (chipsAfterUse.length) {
  ...
  /* ⚠ <b>맨 앞 칩은 갈래가 아니다</b> (2026-08-21 실측). 08-19 `871c40bb5`(KL-332, 사용자 결정)로
     ...
     창이 안 뜬다, 0줄뿐이다, 부모 밑에 안 붙었다, 돌아갈 길이 없다가 <b>한꺼번에</b> 났다.
 ...
 * 명령 팔레트가 실제로 여닫히는지가 내내 빨갰다. 팔레트는 멀쩡히 여닫히는데도.
...
if (!fs.existsSync(path.join(blogRoot, 'apps/blog/t/loan/index.html'))) {
  ...
  if (!rows) problems.push('도구 화면에서 연 팔레트가 대출을 하나도 못 찾는다. 검색 목록이 안 따라왔다');
  await tp.close();
}
