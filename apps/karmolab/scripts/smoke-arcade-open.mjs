 ...
 *
 * 이것만 재면 된다: **한 창이 같이 찾기로 열면, 다른 창의 로비에 그 방이 보이고, 눌러서
 * 들어가진다.** 그 셋 중 하나라도 안 되면 이 기능은 없는 것과 같다.
 *
 * 그리고 하나 더. **같이로 연 방은 목록에 안 뜬다.** 그게 두 단추를 가른 이유다.
 * 이걸 안 재면 공개/비공개가 말로만 남는다.
 *
 * 실제 서버(KarmoLab 봇)를 탄다. 못 닿으면 못 돌았다(2). 통과도 실패도 아니다.
 */
...
/* ★ **여기서는 로비 목록을 못 받는다** (2026-08-14, 계측으로 확인).
   이 검사는 제 서버(127.0.0.1:임의포트)에 앱을 띄우는데, 방 목록은 KarmoLab 봇(다른 출처)에서 온다.
   ...
   **우리 코드가 고장 난 게 아니라 이 자리에서 잴 수 없는 것**이다(실서비스 출처는 고정이라 된다).
   그래서 목록 관련 두 줄은 못 받는 게 확인되면 못 쟀다로 적고 넘어간다 . 
  ...
  /* 방을 닫으면 목록에서도 내려가야 한다. 안 그러면 눌렀는데 아무도 없네가 된다. */
