...
function inspect(label, html, expectUrl) {
  ...
  if (alias) {
    ...
    /* 한 줄 글월로도, 낱말 배열로도 적힌다. 도구 사슬처럼 **띄어쓰기가 든 이름**은
  ...
  /* ★ **글자로 찾으면 그 글자가 옮겨간 순간 없는 병이 된다** (2026-08-17 실측).
     방문 기록기는 오늘 인라인에서 `js/boot-late.js` 로 옮겼다(자물쇠로 가는 길). 그러자
     이 검사가 146장 전부를 기록기가 없다로 잡았다. 실제로는 멀쩡히 돈다(실주소에서
...
/* ★ **한 번 삐끗한 서버를 페이지가 망가졌다로 적지 마라** (2026-08-17 실측). 거울이 이 검사를
   ...
   그래서 5xx, 연결 실패는 **두 번 더** 받아 보고, 그래도 안 되면 못 잼으로 따로 센다(빨강 X).
 ...
 * PDF → 이미지와 이미지 → PDF처럼 비슷한 짝이 생기기 쉬워, 문구를 베끼면 둘 다 손해다.
