...
try {
  ...
  /* ★ **기다렸다가 본다** (2026-08-16). 앱은 서버가 받았다고 답한 **뒤에야** 입력칸을 비운다 . 
     그래야 안 보내진 글이 사라지지 않는다(옳은 동작이다). 그런데 이 검사는 상대 창에 글이
     떴다 다음 줄에서 곧바로 봤다: 상대가 먼저 보는 일이 얼마든지 있다(밀어 주는 연결과
     내 fetch 응답은 다른 길이다). 로컬에선 늘 통과, CI 에선 빨강. **검사가 순서를 가정**했다.
     가정 대신 조건을 기다린다. 그래도 안 비면 그건 진짜 결함이다. */
  await a.page.waitForFunction(
    () => document.querySelector('#klChatInput')?.value === '',
    null,
    { timeout: 5000 },
  ).catch(() => { /* 아래 check 가 실패로 말한다 */ });
  ...
  /* ⑨ **도구 화면에서도 뜬다** (TASK-KL-161).
   * 사람이 실제로 들어오는 곳은 첫 화면이 아니라 도구 화면 129장이다. 예전에는 거기서
   * 계정 스크립트가 없어 채팅이 스스로를 지웠고, 그래서 사이트 어디에나 있다가 거짓이었다.
