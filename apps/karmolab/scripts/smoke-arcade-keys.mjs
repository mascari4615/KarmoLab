...
const openGame = async (id) => {
  ...
  /* ★ **멎었다와 아직 비었다는 다르다** (2026-08-17, 갈아 끼우고 한 판 빨개져서 알았다).
     손으로 짠 옛 고리에는 `지금 > 0` 이 있었다. 무대가 잠깐 빈 순간이 두 번 겹치면 공용 자는
     0 에서 멎었다로 보고 곧바로 키를 누르고, 그러면 아무 데도 안 간다(지난 판에 고친 그 병).
...
if (!cantRun) {
  ...
  for (const id of ['blackjack', 'president']) {
    ...
    /* ★ **누를 것이 생기기 전에 화살표를 누르면 아무 일도 안 난다** (2026-08-17, 라이브 빨강을 짚었다).
       ...
       누를 수 있는 단추가 하나도 없었다. 그래서 짚은 자리가 0 이고 키가 안 먹는다로 빨개졌다.
    ...
    /* ★ **한 번 눌러 안 되면 두 번 더 눌러 본다** (2026-08-17, CI 빨강, 여기선 초록).
       ...
       느린 러너에서만 그 찰나에 걸려 직후 0, 0.5초 뒤 0으로 빨개졌다(실사이트를 이 자리에서
       세 번 눌러 보니 매번 1 이었다. 놀이가 아니라 **누른 때**의 문제다).
       그래도 눌러도 안 된다를 놓치면 안 되므로 **세 번까지만** 눌러 보고, 그 뒤엔 그대로 빨강이다.
    ...
    /* 재움-의도: **0.5초 뒤에도 남아 있나**가 이 판정의 알맹이다. 매 프레임 다시 그리는 놀이에서
       표시가 곧 지워지던 것을 잡으려고 일부러 시간을 흘려보낸다. 기다릴 된 상태가 따로 없다. */
  ...
  /** 값이 **바뀔 때까지** 기다린다. 재우고 읽기는 느린 기계에서 늘 진다. */
  ...
  for (const [id, read] of [
    ...
    /* **벽 안쪽에서 재야 한다.** 한 방향으로 계속 밀면 라켓이 벽에 붙어 시간에 비례가
  ...
  /**
   * **몇 개나 키로 놀 수 있나**. 재는 것을 두 번 틀렸다. 처음엔 단추가 있나로 셌고
   * (판이 선 직후 한 순간만 봐서 43/51), 다음엔 기다리게 고쳤지만(47/51) 여전히 단추였다.
   * 그런데 뱀은 단추가 없어도 **화살표를 직접 받는다**. 단추 있나는 키로 놀 수 있나가 아니다.
   *
   ...
   *
   * 셋 다 키로 놀 수 있나가 아니라 각자 다른 것을 잰 수다. 그걸 문턱으로 걸면 어느 날
   * 애먼 것이 빨개진다(G1 에서 배운 그대로). 그래서 **수는 적어만 두고** 게이트는 위의
   * 실제로 증명된 것(초점 → 화살표 → 엔터 → 돌이 놓임)에만 건다.
   */
  /* ★ **줄 자체가 스스로를 설명해야 한다** (2026-08-17). 위 주석은 이 수는 게이트가 아니다를
