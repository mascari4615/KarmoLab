...
/* 도구 목록, 찾기창, 첫 화면은 **이 표**에서 이름을 읽는다. 위젯이 열리기 전이라
   위젯 안의 t() 는 아직 없다. 그래서 여기서 직접 말 묶음을 받고, 이름은 **읽는 순간에**
   정한다. 아직 안 왔으면 한국어가 그대로 보인다(열쇠 이름이 보이는 것보다 낫다). */
if (typeof document !== 'undefined') {
  /* 빌드가 이 파일을 **브라우저 밖에서** 한 번 돌려 목록을 가른다(build.mjs). 거기엔 document 가
     없으므로 받아오기를 건너뛴다. 그때는 기본값(한국어)이 그대로 쓰인다. */
  void loadNamespace('widgets');
  void loadNamespace('widgets-desc');
}
...
window.KARMOLAB_LAZY_META = [
  ...
  },
  {
     ...
     * 이 화면 자체가 여기 있는 것이 전부 우리가 만든 것은 아니다라는 선언이다. */
    id: 'usertool',
    noPage: true,
  ...
  },
  {
     ...
     * 숨기지 않는다: 이 화면 자체가 도구가 서로 만난다는 이 사이트의 주장이다. */
    id: 'flow',
    noPage: true,
  ...
  },
  {
    /* 커뮤니티는 **부팅에서 뺐다** (TASK-KL-204, 근거 = KL-201 계기판).
       첫 화면에서 77KB 를 받고 한 번도 안 그렸다. 계기판의 받았는데 안 쓴 코드가 잡았다.
  ...
  },
  {
    ...
    lazyScriptPaths: ['mydash/home', 'mydash/me', 'mydash/ai-usage', 'mydash/bookmarks', 'mydash/pc-vitals', 'mydash/career', 'mydash/shell']
  },
  {
    id: 'activity',
    noPage: true,
  ...
  },
  {
    ...
    /* 이 묶음이 내놓는 것 (TASK-KL-183 A). 흐름을 만들 때 이어지는 도구를 위로 올리는 근거.
       숨긴 도구(imagegen 등)에 적어 봐야 목록에 안 뜨므로, **보이는 대표**에 적는다. */
    produces: ['image/*'],
    lazyScriptPaths: ['root/gemini', 'ref/reftable', 'imageconvert/imageconvert', 'imageedit', 'meok/meok', 'tools/imgresize', 'tools/redact', 'tools/gifenc', 'tools/asciiart', 'world/world', 'world/parse-md', 'world/load-characters-from-wiki', 'imagegen/presets', 'imagegen/config', 'imagegen/styles', 'imagegen/core', 'imagegen/imagegen', 'imagelib', 'tools/image']
  },
  {
    id: 'imageedit',
    hidden: true, // 이미지 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
  ...
  /* ★ **작업대로 합친 옛 도구들. 주소는 살려 둔다** (2026-08-13).
     열여섯을 글 작업대의 조작으로 합치면서 낱개 등록이 사라졌고, 그러자 `#textclean` 같은
     **사람 북마크가 아무 데도 안 열리게** 됐다(검사 `smoke:deeplink` 가 그걸 잡고 있었다).
     base64, crypto 가 `devtool` 로 합쳐질 때 쓴 그 방식 그대로. 숨기되 묶음으로 보낸다.
     이름은 작업대 조작의 이름을 그대로 쓴다(말 묶음 열쇠는 합치면서 없어졌다). */
  {
    id: 'slug',
  ...
  },
  {
    ...
    lazyScriptPaths: ['tools/pdftool', 'tools/pdfcrop', 'tools/pdfpagenum', 'tools/pdfredact', 'tools/pdfcompress', 'tools/pdfsign', 'tools/pdfwatermark', 'tools/pdf2text', 'tools/pdf2img', 'tools/img2pdf', 'tools/pdf']
  },
  {
    id: 'sound',
    hidden: true,   // 옆줄과 찾기 판에서 뺌 (2026-08-31 사용자 결정)
  ...
  },
  {
    ...
    // (`/t/hashgen/`)에서만 라이브러리를 불러오지 못했어요가 떴다. 앱 안에서는
  ...
  {
    id: 'reference',
    get title() { return t('widgets.reference.title', undefined, "참고표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.reference.desc', undefined, "git 명령어, 마크다운, HTTP 상태, 단축키, 파일 확장자, 키 코드를 한 곳에서 찾아봅니다"); },
    layout: 'wide',
  ...
  {
    id: 'charmap',
    get title() { return t('widgets.charmap.title', undefined, "문자표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.charmap.desc', undefined, "특수문자, 이모지, HTML 엔티티, ASCII 를 한 곳에서 찾아 눌러 복사합니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    get title() { return t('widgets.specialchar.title', undefined, "특수문자 모음"); },
    category: 'ref',
    get desc() { return t('widgets-desc.specialchar.desc', undefined, "화살표, 별, 도형, 수학기호 등 자판에 없는 특수문자를 눌러서 복사합니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 문자표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'charmap', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.ascii.title', undefined, "ASCII 코드표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.ascii.desc', undefined, "0~127 ASCII 문자의 10진, 16진, 2진 값과 제어문자 의미를 한 표에서 봅니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 문자표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'charmap', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.htmlentity.title', undefined, "HTML 특수문자"); },
    category: 'ref',
    get desc() { return t('widgets-desc.htmlentity.desc', undefined, "&amp;nbsp; &amp;lt; &amp;copy; 같은 HTML 엔티티 코드를 문자와 함께 찾아 복사합니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    get title() { return t('widgets.httpstatus.title', undefined, "HTTP 상태 코드"); },
    category: 'ref',
    get desc() { return t('widgets-desc.httpstatus.desc', undefined, "200, 301, 403, 404, 500 등 HTTP 응답 코드의 뜻과 쓰는 상황을 정리한 표"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 색상 도구 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'color', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.colorname.title', undefined, "CSS 색상 이름표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.colorname.desc', undefined, "CSS 표준 색상 이름 148개와 HEX 값을 눈으로 비교하고 눌러서 복사합니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.keycode.title', undefined, "키보드 이벤트 코드"); },
    category: 'ref',
    get desc() { return t('widgets-desc.keycode.desc', undefined, "키를 누르면 event.key, event.code, keyCode 값을 그 자리에서 보여줍니다"); },
    layout: 'form',
  ...
  },
  {
    ...
    hidden: true, // 문자표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'charmap', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.emoji.title', undefined, "이모지 찾기"); },
    category: 'ref',
    get desc() { return t('widgets-desc.emoji.desc', undefined, "한국어로 검색해서 이모지를 찾고 눌러서 복사합니다. 표정, 손짓, 기호 등 분류별"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.markdown.title', undefined, "마크다운 문법표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.markdown.desc', undefined, "제목, 표, 코드블록, 체크박스 등 마크다운 문법을 찾아 그대로 복사합니다 (GFM 기준)"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.regexref.title', undefined, "정규식 치트시트"); },
    category: 'ref',
    get desc() { return t('widgets-desc.regexref.desc', undefined, "정규식 기호와 자주 쓰는 패턴을 하려는 일로 찾아 복사합니다"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.gitcmd.title', undefined, "git 명령어 모음"); },
    category: 'ref',
    get desc() { return t('widgets-desc.gitcmd.desc', undefined, "하려는 일로 git 명령어를 찾습니다. 되돌릴 수 없는 명령은 따로 표시"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.filetype.title', undefined, "파일 확장자표"); },
    category: 'ref',
    get desc() { return t('widgets-desc.filetype.desc', undefined, "확장자가 무슨 파일이고 무엇으로 여는지 찾아봅니다. 이미지, 문서, 압축, 코드 등"); },
    layout: 'wide',
  ...
  },
  {
    ...
    hidden: true, // 참고표 위젯의 탭으로 합쳐짐. 검색 유입 주소는 유지
    bundle: 'reference', // 이 도구를 부르면 묶음의 이 탭으로 간다
    get title() { return t('widgets.shortcut.title', undefined, "단축키 모음"); },
    category: 'ref',
    get desc() { return t('widgets-desc.shortcut.desc', undefined, "윈도우, 맥, 브라우저, VS Code 단축키 중 알면 실제로 쓰게 되는 것만 모았습니다"); },
    layout: 'wide',
  ...
  /* 오락실 (TASK-KL-242). 실험실 안의 놀이터. 미니게임 여럿을 커널 하나가 굴린다.
     사람이 모자란 자리는 봇이 앉아서 혼자와 같이가 같은 코드다. */
  {
    id: 'arcade',
   ...
   *   타입체크, 번들 정합 검사는 그때도 전부 초록이었다. 없는 것은 검사할 대상이 없으니까. */
  {
    id: 'karmograph',
   ...
   * 못 닿고 있었다(`audit:registry-impl` 이 꺼져 있어 아무도 몰랐다). */
  {
    ...
    layout: 'wide',
    icon:
      '<path d="M5 4h11l3 3v13H5z" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linejoin="round"/>' +
      '<path d="M16 4v3h3" stroke="currentColor" stroke-width="1.6" fill="none"/>' +
  ...
  /* 나만 안 되나?. 위와 같은 사유로 못 닿고 있었다. */
  {
    ...
    icon:
      ...
      '<path d="M5 12.5a10 10 0 0 1 14 0" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round"/>' +
