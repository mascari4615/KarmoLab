...
import {labUrl, onSplitHost} from '../../lib/site-hosts';
...
/** 소개 한 장. 셸 위젯 (`about.ts`) 과 blog 장 (`about-page.ts`) 이 같이 쓴다 */
export async function renderAbout(container: HTMLElement, signal: AbortSignal): Promise<void> {
    const json = async <T,>(name: string): Promise<T> => {
        const response = await fetch(`/apps/karmolab/data/${name}.json`, {signal});
        if (!response.ok) throw new Error(`${name}: ${response.status}`);
        return response.json() as Promise<T>;
    };
    await loadNamespace('about');
    try {
        ...
        root.innerHTML = `<div class="ab-scene" role="img"></div><div class="ab-page">
            <nav class="ab-top"><span></span><a>KarmoLab ↗</a></nav>
        ...
        query('.ab-top a').setAttribute('href', onSplitHost() ? labUrl() : '/');
        ...
        if (!signal.aborted) container.textContent = t('about.failed', undefined, '소개를 못 받았습니다');
    }
}
