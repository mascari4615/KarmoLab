...
void (async function (): Promise<void> {
    ...
    Mdd.injectCSS(
        ...
        .klchat-dock { display:inline-flex; align-items:center; gap:6px; height:32px; padding:0 11px; border:1px solid var(--border); background:transparent; color:var(--text-secondary); border-radius:var(--radius-pill); font-size:var(--font-size-xs,12px); font-weight:600; font-family:inherit; white-space:nowrap; cursor:pointer; transition:color .12s ease, border-color .12s ease, background .12s ease; }
        .klchat-dock:hover { color:var(--text-primary); border-color:var(--accent); }
        .klchat-dock[aria-expanded="true"] { color:var(--accent); border-color:var(--accent); background:var(--accent-dim); }
        .klchat.loose .klchat-dock { height:auto; padding:9px 14px; border-radius:var(--radius-md,8px); background:var(--glass-strong); color:var(--text-primary); font-size:var(--font-size-2xs); }
        .klchat-dot { width:7px; height:7px; border-radius:50%; background:var(--text-tertiary); flex:none; }
        ...
        .klchat-panel { width:min(340px,calc(100vw - 32px)); height:min(460px,calc(100vh - 88px)); display:none; flex-direction:column; border:1px solid var(--border); background:var(--glass-strong); backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px); border-radius:var(--radius-md,8px); overflow:hidden; box-shadow:0 12px 40px var(--vignette); }
        .klchat.open .klchat-panel { display:flex; }
        .klchat-head { display:flex; align-items:center; gap:8px; padding:10px 12px; border-bottom:1px solid var(--border); font-size:var(--font-size-2xs); color:var(--text-secondary); }
        .klchat-head b { color:var(--text-primary); font-size:var(--font-size-2xs); }
        .klchat-head .klchat-spacer { flex:1; }
        .klchat-x { background:none; border:none; color:var(--text-tertiary); cursor:pointer; font-size:var(--font-size-sm); line-height:1; padding:2px 4px; }
        .klchat-x:hover { color:var(--text-primary); }
        ...
        .klchat-line { color:var(--text-primary); word-break:break-word; white-space:pre-wrap; position:relative; padding-right:34px; }
        .klchat-line.cont { padding-left:0; }
        .klchat-who { font-weight:700; }
        .klchat-owner { font-size:var(--font-size-4xs); font-weight:800; padding:0 4px; border-radius:var(--radius-sm); background:var(--accent); color:#04121a; margin-right:4px; vertical-align:1px; }
        .klchat-time { color:var(--text-tertiary); font-size:var(--font-size-4xs); margin-left:6px; opacity:0; transition:opacity .12s; }
        ...
        .klchat-act button { background:none; border:none; padding:0 2px; font-size:var(--font-size-3xs); color:var(--text-tertiary); cursor:pointer; }
        ...
        .klchat-quote { font-size:var(--font-size-3xs); color:var(--text-tertiary); border-left:2px solid var(--border); padding-left:6px; margin:2px 0 1px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .klchat-replying { display:flex; align-items:center; gap:6px; font-size:var(--font-size-3xs); color:var(--text-tertiary); }
        .klchat-replying button { background:none; border:none; color:inherit; cursor:pointer; }
        .klchat-note { color:var(--text-tertiary); font-size:var(--font-size-3xs); line-height:1.6; padding:6px 0; border-bottom:1px dashed var(--border); margin-bottom:4px; }
        .klchat-foot { border-top:1px solid var(--border); padding:8px 10px; display:flex; flex-direction:column; gap:6px; }
        .klchat-row { display:flex; gap:6px; align-items:flex-end; }
        .klchat-input { flex:1; resize:none; height:34px; max-height:96px; padding:8px 10px; border:1px solid var(--border); background:var(--bg-tertiary); color:var(--text-primary); border-radius:var(--radius-sm,6px); font-size:var(--font-size-2xs); font-family:inherit; line-height:1.35; }
        .klchat-input:focus { outline:none; border-color:var(--border-hover); }
        .klchat-send { flex:none; padding:0 12px; height:34px; border:none; border-radius:var(--radius-sm,6px); background:var(--accent); color:#04121a; font-weight:800; font-size:var(--font-size-2xs); cursor:pointer; }
        .klchat-send:disabled { opacity:0.4; cursor:default; }
        .klchat-status { font-size:var(--font-size-3xs); color:var(--text-tertiary); min-height:14px; }
        .klchat-status.warn { color:#ef8b8b; }
        .klchat-alone { border:none; color:var(--text-secondary); }
    ...
    root.innerHTML = `
        <div class="klchat-panel" style="display:none" role="log" aria-label="${t('chat.aria.panel')}">
            <div class="klchat-head">
                ...
                <button type="button" class="klchat-x" id="klChatClose" aria-label="${t('chat.close')}">✕</button>
            </div>
            ...
            <div class="klchat-foot">
                ...
                <div class="klchat-row">
                    <textarea class="klchat-input" id="klChatInput" rows="1" placeholder="${t('chat.placeholder')}" maxlength="300"></textarea>
                    <button type="button" class="klchat-send" id="klChatSend">${t('chat.send')}</button>
                </div>
    ...
    dock.innerHTML = `
        ...
        <span id="klChatDockCount" style="color:var(--text-tertiary);font-weight:600"></span>
    ...
    function lineHtml(m: Message, previous: Message | null): string {
        ...
        const head = sameSpeaker
            ? ''
            : `${owner}<span class="klchat-who" style="color:${m.color}">${escapeHtml(m.name)}</span><span style="color:var(--text-tertiary)">: </span>`;
        ...
        const report = m.reportedByMe
            ? `<button data-act="none" data-on="1" title="${t('chat.reportedAlready')}" disabled>🚩</button>`
            : `<button data-act="report" data-id="${m.id}" title="${t('chat.report')}">🚩</button>`;
        const actions = isAdmin
            ? `${answer}${keep}<button data-act="del" data-id="${m.id}" title="${t('chat.delete')}">🗑</button><button data-act="mute" data-who="${m.who}" title="${t('chat.mute')}">🤫</button>`
            : `${answer}${keep}${report}`;
    ...
    function renderLog(): void {
        ...
        const hint =
            pref(HINT_KEY, '') === '1'
                ? ''
                : `<div class="klchat-note">${t('chat.hint')}</div>`;
        ...
        if (onlyKept && shown.length === 0) {
            html += `<div class="klchat-note" style="border:none">${t('chat.emptyKept')}</div>`;
        }
        if (!onlyKept && messages.length === 0) {
            html += `<div class="klchat-note" style="border:none">${t('chat.emptyAll')}</div>`;
        }
    ...
    function renderReplying(): void {
        ...
        el.replying.innerHTML =
            ...
            `<button type="button" data-cancel-reply title="${t('chat.cancelReply')}">✕</button>`;
    ...
    /** 새 줄이 왔을 때 점을 한 번 퍼뜨린다. 평소에는 안 돈다(가만히 있는 화면은 쉬어야 한다). */
    function pingDot(): void {
        for (const dot of [el.dot, el.dockDot]) {
            if (!dot) continue;
            dot.classList.remove('ping');
            void dot.offsetWidth;          // 연달아 오면 다시 처음부터 돌게 한다
    ...
    function connect(): void {
        ...
        source.onerror = () => {
            ...
            connected = false;
            renderHeader();
            el.status.textContent = t('chat.reconnecting');
    ...
    const failureText = (): Record<string, string> => ({
        ...
        too_fast: t('chat.err.tooFast'),
        too_many: t('chat.err.tooMany'),
        muted: t('chat.err.muted'),
        not_human: t('chat.err.notHuman'),
    });
    ...
    async function send(): Promise<void> {
        ...
        try {
            ...
            const wait = body.retryAfterMs
                ? ' ' + t('chat.err.retryAfter', { sec: Math.ceil(body.retryAfterMs / 1000) })
                : '';
            el.status.textContent = (failureText()[body.error ?? ''] ?? t('chat.err.failed')) + wait;
            el.status.classList.add('warn');
        } catch {
            el.status.textContent = t('chat.err.offline');
            el.status.classList.add('warn');
        } finally {
    ...
    function keep(id: string): void {
        ...
        Toolbox.showToast?.(t('chat.moved'), 'info');
    }
    ...
    el.input.oninput = () => {
        ...
        el.status.classList.remove('warn');
        el.status.textContent = left <= 40 ? t('chat.left', { n: left }) : '';
    };
    ...
    el.log.onclick = (event) => {
        ...
        if (kind === 'report') {
            ...
            Toolbox.showToast?.(t('chat.reported'), 'info');
        }
