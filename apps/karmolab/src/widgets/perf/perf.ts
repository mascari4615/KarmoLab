 ...
 * 메모리와 긴 작업을 못 잰다. 그 자리에 0 을 적으면 아주 좋다로 읽혀서 고칠 것을 못 찾는다.
 */
...
(function (): void {
  ...
  Mdd.injectCSS(
    ...
    `
        ...
        .pf-card-name { font-size:var(--font-size-3xs); color:var(--text-tertiary); margin-bottom:5px; }
        .pf-card-value { font-size:var(--font-size-md); font-weight:700; color:var(--text-primary);
            font-variant-numeric:tabular-nums; }
        .pf-card-note { font-size:var(--font-size-3xs); color:var(--text-tertiary); margin-top:4px; }
        .pf-sec-title { margin:0 0 4px; font-size:var(--font-size-md); font-weight:700; color:var(--text-primary); }
        .pf-sec-note { margin:0 0 10px; font-size:var(--font-size-3xs); color:var(--text-tertiary); line-height:1.6; }
        ...
        .pf-fall-track { height:12px; background:var(--bg-tertiary); border-radius:var(--radius-pill); position:relative;
            overflow:hidden; }
        .pf-fall-fill { position:absolute; top:0; bottom:0; background:var(--accent); opacity:.75;
            border-radius:var(--radius-pill); min-width:2px; }
    ...
    tabs: [
      {
        ...
        build(container: HTMLElement): void {
          ...
          /* 계측기가 안 실린 판이면 **표를 0 으로 채우지 않는다**. 전부 0ms는 아주 빠른
           ...
           * 못 읽으면 없다가 아니라 **못 읽었다**고 적는다(파일이 없는 판일 수 있다).
           */
           ...
           *
           * 이 줄이 실행됐나는 브라우저가 페이지에 안 알려 준다. 개발자 도구 전용 통로다.
           ...
           *
           * 이 화면의 다른 숫자는 전부 지금 이 기기다. 만든 사람 기계에서 잰 값이라
          ...
          function interactionTable(snap: Snap): string {
            ...
            /* 표시 40ms = 느리다는 틀린 판정이다. 핸들러가 하나도 없는 빈 클릭도 이 환경에선
           ...
           * 긴 작업 표는 언제 얼마나까지고, 이 표는 어느 파일의 어느 함수가다.
           */
           ...
           * 브라우저는 이 줄이 실행됐나를 페이지에 안 알려 준다 → 우리가 아는 것으로 근사한다.
           */
          ...
          /** 판별 분포. 한 번 재고 빨라졌다는 말은 못 한다 (TASK-KL-201 ⑦). */
            ...
            return `<div class="pf-scroll"><table class="pf-table">
              ...
              <tbody>${rows
                .map((row) => {
                  ...
                  /* `toLocaleString` 은 8. 9. 13시 19분 31초로 나온다. 표에서 자리가 들쭉날쭉해
           ...
           * 한 줄 판정 (TASK-KL-201 ⑧).
           *
           * 숫자만 늘어놓으면 볼 때마다 이게 좋은 건가를 사람이 다시 판단해야 한다.
           * **못 잰 것은 합격으로 안 센다**. 합격과 검사 못 함을 같은 칸에 넣으면
           ...
           * 잡혔다: 다시 재기의 **표시 지연 70ms**. 재는 도구가 재는 대상을 흔들면
          ...
          void fetch('https://bot.mascari4615.com/kl/tools/stats', { credentials: 'include' })
            ...
            .catch(() => {
              /* 서버가 자고 있어도 계기판은 그대로 돈다. 그 칸만 못 받았다로 남는다. */
              realState = 'fail';
            })
            .then(() => render());
          ...
          /* 이 화면 **자기 줄**은 첫 그림 때 비어 있다 (TASK-KL-201).
             순서가 그렇다: 스크립트를 받는 도중에 등록, 그리기가 일어나고, 눌러서 준비까지는
             그 받기가 다 끝난 뒤에야 적힌다. 그래서 자기 줄만 크기, 받기가 . 로 남는다 . 
             사람이 다시 재기를 눌러야 채워지는 건 계기판으로서 부끄럽다. 한 박자 뒤 한 번만
