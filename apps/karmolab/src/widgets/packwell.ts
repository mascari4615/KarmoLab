...
(function (): void {
  ...
  /** 기존 오늘의 문제와 같은 대조 규칙. 소문자, 공백, 쉼표를 지우고 잰다. */
  ...
  Toolbox.register({
    ...
    tabs: [
          ...
          void loadNamespace('packwell').then(() => {
            ...
            ask<{ wells: WellRow[]; today: string; day: string }>('/kl/wells', control.signal)
              .then((body) => {
                ...
                /* 오늘의 표를 맨 위로 올린다. 무엇부터 눌러야 하나가 없으면 다섯 개가
             ...
             *
             * 애니 vs 게임처럼 원래 견줄 수 없는 것들을 한 판에 올린다. 메인이 아니라서
             ...
             *
             * 지금 1등은 아무나 보여 준다. **지난주보다 뭐가 올라왔나**는 우리가 쌓아 둔
            ...
            function take(pack: WellPack, already: Pack | null): void {
              ...
              // 놀이가 열릴 때 **이 표가 이미 골라져 있게** 밀어 넣는다 (내 표가 쓰는 길과 같다).
              putPick(saved.id);
               ...
               * 못 노는 놀이는 아예 안 보여 준다. 숫자 칸이 없으면 높은 쪽 고르기는 그 표를
      ...
      },
      {
         ...
         * 답이 안 보이게. 대조 규칙은 기존 오늘의 문제와 같다(소문자, 공백, 쉼표 지우기).
         */
        build: function (root: HTMLElement): void {
          /* **받아온 뒤 그린다**. 말 묶음보다 먼저 그리면 한국어가 한 번 스친다. */
          void loadNamespace('packwell').then(() => {
