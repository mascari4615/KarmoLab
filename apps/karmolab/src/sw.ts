...
const PRECACHE_IDLE = [
  ...
  /* ★ **서버에 못 닿는다는 쪽지가 그 쪽지를 담은 파일과 함께 사라졌다** (2026-08-14).
  ...
  /* ★ **그 쪽지를 불러오는 파일도 같이 담아야 한다** (2026-08-19). 위의 `account.js` 를
     담고도 쪽지는 안 떴다. 그 파일을 부르는 것이 `boot-late.js` 이기 때문이다.
     예전엔 머리에 박힌 인라인 줄이 불렀으니(끊겨도 항상 있었다) 드러나지 않았고,
     자물쇠(CSP) 때문에 밖으로 빼면서(2026-08-17) 그 줄이 한 파일이 됐다 . 
     끊긴 채 열면 그 파일부터 안 와서 **쉬는 것을 말해 주는 쪽지가 영영 안 떴다**
     (실측: `test:offline`. 끊고 8초를 기다려도 없음). 무겁지도 않다(2KB). */
  '/apps/karmolab/js/boot-late.js',
  /* ★ **앱이 뜨는 데 꼭 필요한 것들** (2026-08-14). 끊긴 채 도구 주소로 들어가면 껍데기는
     오는데 **화면이 하나도 안 그려졌다**. 계측으로 보니 `Toolbox` 가 아예 없었다.
     워커는 지나간 것만 담으므로, 그 사람이 그 판에 안 불러 본 파일은 없다.
...
ctx.addEventListener('install', (event: ExtendableEvent) => {
  ...
  // skipWaiting 없음. 사용자가 지금 갱신을 누를 때까지 기다린다.
});

ctx.addEventListener('activate', (event: ExtendableEvent) => {
  event.waitUntil(
    (async () => {
      // 화면 이동을 이 워커가 가로채면, 워커가 깨어나는 동안 요청이 멈춰 있다. 미리 보내기를
 ...
 * 그래서 서비스 워커가 그 자리를 대신한다(표준 방식): 파일을 받아 이어서가 쓰는 그 칸에
      ...
      Promise.resolve(preloaded)
        ...
        .catch(() =>
          caches.match(req).then((cached) => {
             ...
             *
             * 받아 둔 것은 첫 화면(`/`) 한 장뿐이라, 설치한 앱에서 도구 주소로
