/**
 * 공개 프로필 페이지 (TASK-KL-098 Cycle 1). `/u/?h=<핸들>`.
 *
 * 북적북적이 실제로 보이는 첫 자리다. **로그인 없이 남이 열려야** 의미가 있다 . 
 * 그래서 로그인은 있으면 좋은 것이지 있어야 하는 것이 아니다.
 * 프로필 본문 요청에만 쿠키를 함께 보낸다 (TASK-KL-152 C8): 내가 이 사람을 따라가고 있나는
 * 보는 사람이 누구냐에 따라 답이 달라서, 그것 없이는 단추를 그릴 수 없다.
 * 로그인 안 한 사람에게는 지금까지와 똑같이 보인다(단추만 없다).
 *
 * 왜 `?h=` 인가: 이 사이트는 정적으로 올라간다. `/u/<핸들>/` 같은 주소는 핸들마다
...
interface PublicProfile {
    ...
    /** 본인이 일부러 가린 칸 (TASK-KL-152 C4). 아직 없다와 다르다. */
    hidden?: string[];
 ...
 *
 * 여태 프로필은 아무나 똑같이 생겼다(이름, 아바타, 숫자뿐). 이 두 줄이 있어야 누구인지가 보인다.
 ...
 * 빈 칸을 그냥 두면 보는 사람이 아무것도 안 했나 보다로 읽는다. 그건 틀린 말이다.
 */
 ...
 * 가렸거나 기록이 없으면 통째로 안 그린다(빈 잔디는 안 왔다로 읽혀 거짓말이 된다).
 * 안 온 날과 둘러보기만 한 날을 다르게 칠하는 규칙은 내 화면과 같다.
 */
...
async function loadWorks(handle: string): Promise<void> {
    ...
    try {
         ...
         * 않는다**. 부르면 깨진 칸이 남고, 그건 안 걸린 것처럼 보인다. 대신 갈래 표시와
...
/** 이 사람이 쓴 글, 답글. 무엇을 했나가 없으면 프로필은 빈 명함이다. */
...
async function main(): Promise<void> {
    ...
    try {
        ...
        // 서버가 잠깐 죽은 것과 없는 사람은 다르다. 섞어서 말하지 않는다.
