...
namespace KarmoLab.YawnBot.Modules
{
	public class StockModule : InteractionModuleBase<SocketInteractionContext>
	{
		...
		{
			EmbedBuilder embed = _stockService.GetStockListEmbed();
		...
		{
			(bool success, string message) = _stockService.BuyStock(Context.User.Id, symbol.ToUpper(), amount);
		...
		{
			(bool success, string message) = _stockService.SellStock(Context.User.Id, symbol.ToUpper(), amount);
			await RespondAsync(message, ephemeral: !success);
		}
		...
		{
			EmbedBuilder embed = _stockService.GetMyStockEmbed(Context.User.Id, Context.User.Username);
			await RespondAsync(embed: embed.Build());
		}
	}
}
