...
namespace KarmoLab.YawnBot.Modules
{
	public class RaidModule : InteractionModuleBase<SocketInteractionContext>
	{
		...
		public async Task AttackAsync()
		{
			...
			(long damage, bool isDead, bool isCritical) = _raidService.Attack(Context.User.Id, Context.User.Username);
			...
			{
				msg += "\n\n💀 **보스가 처치되었습니다!**";
				EmbedBuilder embed = _raidService.GetStatusEmbed();
				await RespondAsync(msg, embed: embed.Build());
			}
			else
			{
				YawnBot.Models.RaidBoss? boss = _raidService.CurrentBoss;
				if (boss != null)
				{
