...
const askFirst = {
  confirm(what) {
    return new Promise((resolve) => {
      ...
      // 아무도 대답 안 하면 30초 뒤 아니오로 본다. 영원히 매달려 있지 않게.
      setTimeout(() => {
...
// 두뇌에 넘어가는 건 최근 몇 마디뿐이라, 이게 없으면 저번에 그거를 영영 모른다.
...
// 조용히 있기. 좀 있다 얘기해라고 말할 수 있게. 밤(23~7시)도 조용한 시간이다.
// 발동 기록. 만든 게 실제로 도는지 센다. 도는지 모름과 스무 번 중 0번은 다른 말이다.
...
/* 자리를 배운다. 표가 모르는 창은 두뇌에게 물어 적어 둔다(78회차).
   지금 떠 있는 창을 세어 보니 아홉 중 여섯이 모름이었다. 표를 늘리는 건 답이 아니다 . 
   사람이 쓰는 프로그램은 사람마다 다르다. 같은 창은 한 번만 물어보므로 값이 곧 0 에 준다. */
/* 되새김. 오간 말에서 **바로 안 보이는 것**을 스스로 짚는다(79회차).
   여태 기억은 셋 다 일어난 일이었다. 그 위에 얹히는 것, 그래서 뭐가 보이나가 없었다.
...
function shouldTossBack() {
  ...
  // **왜 안 하는지 남긴다.** 빔만 보이면 네 kind 중 어디서 빠졌는지 몰라 실험을
...
function startStubServer() {
   ...
   * 목소리가 안 나온다로만 보이고 이유는 아무 데도 안 적혔다(2026-08-19 실측:
...
/* 마이크로 들어온 판들의 결(길이, 크기). **평소**를 알아야 평소보다를 말할 수 있다 . 
...
/* 방금 들어온 판의 결. 재료를 만드는 자리는 감각을 직접 못 보므로 여기 담아 둔다 . 
   **담는 자리(재료 만들기 직전)와 읽는 자리가 같은 turn 안에서 앞뒤로 붙어 있다.**
   70회차에 같은 값을 두 곳에서 들고 있으면 어긋난다를 겪었기에, 담는 자리를 하나로 못 박는다. */
...
const web = webBody({
  ...
  /* 밤엔 목소리도 밤답게.
     ...
     조용한 시간엔 마음이 어떻든 누그러진 결로 말한다. 조용히 있어 달라고 부탁받은
  ...
  tally: () => {
    /* 되묻게 했다는 만든 사람 말이고 **몇 번 중 몇 번인가**가 결과다. 재료만 얹어
...
${tallyReport(tally)}`;
  ...
  speech: await (async () => {
    ...
    /* 로컬 목소리는 메모 저장소 안에 있다. 여기서 세 겹 위로 박아 두면 워크트리에서
    ...
    if (stubReferenceAudio() !== null || process.env.COMPANION_CLONE_REF) {
      ...
      /* **없어진 걸 없어졌다고 말한다.** 여태 안 떠 있으면 목록에서 조용히 빠졌고, 그건
         화면에서 목소리가 회귀했다로만 보였다(실측 2026-08-08). 오늘 사고 셋(로컬
...
/* 관객이 있는 자리. 토큰을 주면 디스코드에도 몸이 선다(코어는 그대로다).
   **토큰이 없으면 그렇다고 말한다.** 조용히 안 붙으면 왜 디스코드에서 아무 말도 없지가
...
// 몸을 따로 두는 이유는 통로를 갈라야 말과 닿음이 안 섞이기 때문이다.
...
const mouth = mouthGate({
  ...
  alsoRetryWhen: (text) => severeDrift(text)
    /* **심하게 샌 말은 나가기 전에 막는다** (127회차). 표류 감시는 원래 다음 번에 일러
       주기라 말투 미세 표류에는 그게 맞지만, 저는 Claude인데... 같은 건 다음 번엔 늦다 . 
       그 말이 이미 조수님한테 간 뒤다. 여기서 막는 건 누구인지, 어느 세계 말인지 둘뿐이다. */
    ?? unbackedClaim(text, writer)
    /* 누를게라고 말만 하고 안 누른 것도 여기서 잡는다. 122회차 라이브에서 실제로
  ...
  // 왜 다시 시키는지에 따라 시키는 말이 다르다. 결에서 벗어났다와 알맹이가 없다는
  // 고칠 데가 다르다.
  retry: (why) => {
    // **다시 시켰다는 것 자체가 안 보였다.** 걸러진 것만 기록에 남아서, 다시 시켰는데
    // 또 안 됐다와 애초에 안 걸렸다를 구분할 수가 없었다.
...
const companion = new Companion({
  ...
  beforeSpeak: async (text, ctx) => {
    ...
    found2 = ctx.found ?? [];
    // **손도 센다.** 재료만 세고 손은 안 세면 손을 안 쓴다가 관찰 두 번짜리 인상으로
  ...
  /* 되돌릴 수 없는 손은 여기서 묻는다. 손마다 감싸지 않고 한 자리에서.
     30초 안에 대답이 없으면 아니오다(askFirst 가 그렇게 만들어져 있다). */
  askBeforeRisky: {
  ...
  recall: async (sensation, recent) => {
    ...
    const old = canSearch
      ...
      : [];
    /* 낱말로 못 찾은 것을 뜻으로 한 번 더. 매운 거 싫어와 마라탕은 못 먹어는
    ...
    // **파일로 더한 손의 힌트가 먼저다.** 사람이 이럴 때 쓰라고 적어 둔 것이니
  ...
  mood: (recent) => {
    ...
    /* 두뇌에 실제로 뭐가 들어가는지 눈으로 본다 (`COMPANION_SHOW_MATERIAL=1`).
       대화가 되는 느낌이 아니다가 **모델 탓인지 재료 탓인지**는 재료를 봐야 갈린다 . 
    ...
    /* 드물게 켜지는 재료는 켜졌을 때 이겨야 한다(82회차). 얼마나 드문지는 우리가 정하지
       ...
       참은 만큼, 하나는 드문 만큼이라 서로 다른 것을 잰다. */
    const materials = [
      /* 방금 알게 된 것. 예전부터 알던 척을 막는다.
         2분 전에 처음 들은 걸 예전부터 알았잖아라고 하면, 사람은 그 순간 얘가
      ...
      { name: '고침', weight: 13, text: (() => {
        ...
        // 안 그러면 창을 새로 연 뒤 첫 아니야가 통째로 무시된다(실측 33회차).
      ...
      /* **제 성적을 얘가 본다** (130회차). 점수판을 셋이나 지었는데 전부 우리가 재는
         것이었다. 얘 자신은 제가 어떤지 모른다. 밖에서 아는 것과 하는 것의 틈이라
      ...
      // 말이 서넛도 안 들어 있어, 판단이 서기도 전에 늘 빔이 된다(실측 36회차).
      { name: '말버릇', text: rutWarning(wholeStory), weight: 10 },
      // 자기 얘기를 물었을 때만. 안 물었는데 전에 이렇게 말했다를 늘 얹을 이유가 없다.
      ...
      // 마지막 대화 = 방금이 되어 **늘 이어짐**이 된다. 실제로 그래서 안 켜졌다
      ...
      /* **사람이 실제로 반응한 것.** 가중치가 최하위(3)라 194번 밀리고 13번만 실렸다
        . 발동 기록으로 확인. 그런데 이건 무슨 말이 통했나라는, 이 사람한테만
  ...
  attention: tactfulAttention({
    // 닿은 것은 나한테 직접 한 짓이다. 지금 바쁘신 것 같아 참았다가 말이 안 된다.
  ...
  onCycle: (report) => {
    /* **두뇌가 터진 turn 을 사람 눈앞에 띄운다.**
       ...
       매번 침묵했고, 창에는 아무 것도 안 떴다. 밖에서는 그냥 가끔 말을 안 한다였다.
       실제 사연은 .env 에 GEMINI_API_KEY 가 필요합니다였다(103회차 실측).
    ...
    if (report.sensation.channel === 'web') {
      ...
      /* 이번 판의 목소리 결을 평소값에 보탠다. 다음 판이 평소보다를 말할 수 있게. */
    ...
    /* 말이 얼마쯤 쌓이면 한 번 되새긴다. 매 turn 하면 그게 값이고, 안 하면 얘는
       영영 일어난 일만 안다. 여기는 이미 말이 나간 뒤라 늦어져도 대화가 안 밀린다. */
...
process.on('SIGINT', async () => {
  ...
  // 끄기 전에 한 번 접는다. 방금 나눈 말이 아는 것에 안 남고 사라지지 않게.
