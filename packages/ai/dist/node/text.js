...
exports.generateAiStudioChatText = generateAiStudioChatText;
 ...
 */
function resolveGeminiModelId(opts) {
    const explicit = opts.modelId?.trim();
    if (explicit)
        return explicit;
    if (opts.tier)
        return (0, index_1.getGeminiModelIdForTier)(opts.tier, opts.env);
    const envOverride = opts.env?.GEMINI_MODEL?.trim();
    if (envOverride)
        return envOverride;
    return index_1.DEFAULT_TEXT_MODEL_ID;
}
...
async function generateAiStudioText(opts) {
    ...
    const usage = (0, index_1.parseGeminiUsageMetadata)(response.usageMetadata);
...
async function generateVertexText(opts) {
    ...
    const safetySettings = (0, index_1.buildGeminiSafetySettings)(opts.safetyThreshold);
    ...
    const usage = (0, index_1.parseGeminiUsageMetadata)(data.usageMetadata);
    return { text, usage, modelId };
}
function readSurfaceRaw(env) {
    ...
    if (env.KARMOLAB_AI_SURFACE?.trim() && !env.KARMO_AI_SURFACE?.trim()) {
        throw new Error('.env 가 옛 이름을 쓰고 있다: KARMOLAB_AI_SURFACE → KARMO_AI_SURFACE 로 바꿔라'
            + ` (지금 값: ${env.KARMOLAB_AI_SURFACE.trim()})`);
    }
    return (env.KARMO_AI_SURFACE?.trim() ||
...
async function generateBlobTextFromEnvWithOptions(env, blobPrompt, options = {}) {
    ...
    if (surface === 'vertex') {
        ...
        (0, index_1.recordUsage)(usage, { ...callMeta, modelId, durationMs: Date.now() - t0, ts: new Date().toISOString() }, options.onUsage);
 ...
 */
function tryCreateGenerativeTextFromEnv(env = process.env, opts = {}) {
